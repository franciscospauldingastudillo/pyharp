...
/* Given an array xx[1..n], and given a value x, returns a value j such that x
   is between xx[j] */
/* and xx[j+1]. xx must be monotonic, either increasing or decreasing. j=0 or
   j=n is returned   */
/* to indicate that x is out of range. A unit-offset array xx is assumed. To use
   locate_disort with    */
...
/* From "Numerical recipes in C" */
{
  ...
  while (ju - jl > 1) {  /* If we are not yet done,             */
    ...
    if ((x >= xx[jm] && monotonicity == 1) ||
        (x <= xx[jm] && monotonicity == 0))
...
/* From "Numerical recipes in C" */
{
  ...
  while (ju - jl > 1) {  /* If we are not yet done,             */
    jm = (ju + jl) >> 1; /* compute a midpoint,                 */
                         /*    fprintf(stderr,"x %e >= xx %e; ju %d jm %d jl
                          * %d\n",x,xx[jm],ju,jm,jl);*/
    if ((x >= xx[jm] && monotonicity == 1) ||
        (x <= xx[jm] && monotonicity == 0))
      jl = jm; /* and replace either the lower limit  */
