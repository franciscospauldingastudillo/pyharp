...
typedef struct {
  ...
  int usrtau, /* TRUE=> radiant quantities returned at user-specified optical
                 depths  */
      usrang, /* TRUE=> radiant quantities returned at user-specified polar
                 angles    */
      ...
      onlyfl, /* FALSE=>return intensities in addition to other radiant
                 quantities    */
      ...
      general_source, /* Include solution for a general user specified source
                         term. */
...
typedef struct disort_state {
  ...
  int nlyr, /* Number of computational layers */
      ...
      nmom_nstr, /* IMAX(ds.nmom,ds.nstr), used to set size of 1st dimension of
                    PMOM     */
  ...
  double wvnmlo, /* Wavenumber [cm^-1] lower range, used for Planck function */
      ...
      radius; /* Radius of the body of interest. Only used if spher=TRUE. Must
                 be     */
  ...
  double *dtauc, /* Optical depths of computational layers, DTAUC(lc) */
      ...
      *utau,   /* Optical depths, in increasing order, on layer boundaries,
                  UTAU(lu)  */
...
typedef struct disort_output {
  ...
  double *albmed, /* Albedo of medium, ALBMED(iu) (ds.flag.ibcnd = SPECIAL_BC
                     case only)  */
      *trnmed,    /* Transmissivity of medium, TRNMED(iu) (ds.flag.ibcnd =
                     SPECIAL_BC)    */
      *uu, /* Intensity, UU(iu,lu,j) (if ds.flag.onlyfl = FALSE; zero otherwise)
            */
      *u0u, /* Az.av.Int. U0U(iu,lu,j) (if ds.flag.onlyfl = FALSE; zero
               otherwise)  */
...
typedef struct {
  double xb_0d, /* x-sub-zero-sup-minus in expansion of pseudo-spherical beam
                   source, Eq. KST(22) */
      xb_0u,    /* x-sub-zero-sup-plus  "    */
      xb_1d,    /* x-sub-one-sup-minus  "    */
      xb_1u,    /* x-sub-one-sup-plus   "    */
      xp_0, /* x-sub-zero in expansion of thermal source func.; Eq. KST(22), has
               no (mu) dep. */
      xp_1, /* x-sub-one  " */
      yb_0d, /* y-sub-zero-sup-minus in Eq. KST(23), solution for
 ...
 *
 * NOTE: ARRAY(iq,jq) is defined locally instead of here, because its size is
 * different in different subroutines.
 */
...
#define GENSRC(maz, lc, iq) \
  ds->gensrc[iq - 1 + (lc - 1 + maz * ds->nlyr) * ds->nstr]
#define GENSRCU(maz, lc, iu) \
...
#define OUT_UUM(iu, lu, j)             \
  out->uum[iu - 1 +                    \
           (lu - 1 + (j) * ds->ntau) * \
               ds->numu] /* No -i behind j as mazim starts at 0, aky */
...
/*
 * NMUG : Number of angle cosine quadrature points on (-1,1) for integrating
 * bidirectional reflectivity to get directional emissivity (it is necessary to
 * use a quadrature set distinct from the computational angles, because the
 * computational angles may not be dense enough---ds->nstr may be too small---to
 * give an accurate approximation for the integration).
 */
#define NMUG 50
...
void c_interp_coefficients_beam_source(disort_state *ds, double *chtau,
                                       ...
                                       double *xba, double *zj, double *ylm0,
                                       double *ylmu);
