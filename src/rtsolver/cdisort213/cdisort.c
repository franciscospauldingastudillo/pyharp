 ...
 *
 * The C-version has the following extensions NOT present in the original
 *fortran version:
 *  - Correction of intensity fields by Buras-Emde algorithm, included by Robert
 *Buras.
 ...
 *
 * This file, cdisort.c, contains the source code for disort(), twostr(), and
 *supporting subroutines. The corresponding header file is cdisort.h.
 *
 * See DISORT.txt for a full description and history of the Fortran version.
 *
 *-----------------------REFERENCES (cited in code using the acronyms
 *shown)------------------------------------
 *
 *     BDE: Buras R, Dowling T, Emde C, 201X, ...
 *    DGIS: Devaux C, Grandjean P, Ishiguro Y, Siewert CE, 1979,
 *              On Multi-Region Problems in Radiative Transfer, Astrophys. Space
 *Sci. 62, 225-233 GS: Garcia RDM, Siewert CE, 1985, Benchmark Results in
 *Radiative Transfer, Transport Theory and Statistical Physics 14, 437-483 KS:
 *Kylling A, Stamnes K, 1992, Efficient yet accurate solution of the linear
 *transport equation in the presence of internal sources: The
 *exponential-linear-in-depth approximation, J. Comp. Phys., 102, 265-276 L:
 *Lenoble J, ed, 1985:  Radiative Transfer in Absorbing and Scattering
 *Atmospheres: Standard Computational Procedures, Deepak Publishing, Hampton,
 *Virginia NT: Nakajima T, Tanaka M, 1988,  Algorithms for Radiative Intensity
 *Calculations in Moderately Thick Atmospheres Using a Truncation Approximation,
 *J.Q.S.R.T. 40, 51-69 OS: Ozisik M, Shouman S, 1980,  Source Function Expansion
 *Method for Radiative Transfer in a Two-Layer Slab, J.Q.S.R.T. 24, 441-449 SS:
 *Stamnes K, Swanson R, 1981,  A New Look at the Discrete Ordinate Method for
 *Radiative Transfer Calculations in Anisotropically Scattering Atmospheres, J.
 *Atmos. Sci. 38, 387-399 SD: Stamnes K, Dale H, 1981, A New Look at the
 *Discrete Ordinate Method for Radiative Transfer Calculations in
 *Anisotropically Scattering Atmospheres. II: Intensity Computations, J. Atmos.
 *Sci. 38, 2696-2706 S1: Stamnes K, 1982, On the Computation of Angular
 *Distributions of Radiation in Planetary Atmospheres, J.Q.S.R.T. 28, 47-51 S2:
 *Stamnes K, 1982, Reflection and Transmission by a Vertically Inhomogeneous
 *Planetary Atmosphere, Planet. Space Sci. 30, 727-732 SC: Stamnes K, Conklin P,
 *1984, A New Multi-Layer Discrete Ordinate Approach to Radiative Transfer in
 *Vertically Inhomogeneous Atmospheres, J.Q.S.R.T. 31, 273-282 SW: Sweigart A,
 *1970, Radiative Transfer in Atmospheres Scattering According to the Rayleigh
 *Phase Function with Absorption, The Astrophysical Journal Supplement Series
 *22, 1-80 STWJ: Stamnes K, Tsay SC, Wiscombe W, Jayaweera K, 1988, A
 ...
 *    STWL: Stamnes K, Tsay SC, Wiscombe W, Laszlo I: A General-Purpose
 *Numerically Stable Computer Code for Discrete-Ordinate-Method Radiative
 *Transfer in Scattering and Emitting Layered Media, DISORT Report v1.1 (2000)
 * VH1,VH2: Van de Hulst, H.C., 1980: Multiple Light Scattering, Tables,
 *Formulas and Applications, Volumes 1 and 2, Academic Press, New York. W:
 *Wiscombe, W., 1977:  The Delta-M Method: Rapid Yet Accurate Radiative Flux
 *Calculations, J. Atmos. Sci. 34, 1408-1422
 *-----------------------------------------------------------------------------------------------------------------
 *
 * This file contains the following functions:
 *
 *   c_disort()...................Plane-parallel discrete ordinates radiative
 *transfer program c_bidir_reflectivity().......Supplies surface bi-directional
 *reflectivity (Fortran name bdref). c_getmom()...................Calculate
 *phase function Legendre expansion coefficients in various special cases.
 *   c_asymmetric_matrix()........Solve eigenfunction problem for real
 *asymmetric matrix known a priori to have real eigenvalues (Fortran name
 *asymtx). c_intensity_components().....Calculate the Fourier intensity
 *components at the quadrature angles for azimuthal expansion terms (mazim) in
 *eq. SD(2), STWL(6) (Fortran name cmpint).
 *   c_fluxes()...................Calculate the radiative fluxes, mean
 *intensity, and flux derivative with respect to optical depth from the m=0
 *intensity components (the azimuthally-averaged intensity).
 *   c_intensity_correction().....Correct intensity field by using
 *Nakajima-Tanaka (1988) algorithm (Fortran name intcor).
 *   c_secondary_scat()...........Calculate secondary scattered intensity of eq.
 *STWL (A7) (Fortran name secsca). c_new_intensity_correction().Correct
 *intensity field by using Buras-Emde (201X) algorithm (Fortran name intcor3).
 *   prep_double_scat_integr()....Prepare integration for double scattering
 *intensity correction (see BDE). c_new_secondary_scat().......Calculate
 *secondary scattered intensity of eq. BDE (XX) (Fortran name secsca3).
 *   calc_phase_squared().........Calculate double scattering intensity
 *correction (see BDE). c_disort_set()...............Perform misc. setting-up
 *operations (Fortran name setdis). c_set_matrix()...............Calculate
 *coefficient matrix for the set of equations obtained from the boundary
 *                                conditions and the
 *continuity-of-intensity-at-layer-interface equations (Fortran name setmtx).
 *   c_single_scat()..............Calculates single-scattered intensity from
 *eqs. STWL (65b,d,e) (Fortran name sinsca). c_solve_eigen()..............Solve
 *eigenvalue/vector problem necessary to construct homogeneous part of discrete
 *ordinate solution; STWJ(8b), STWL(23f) (Fortran name soleig).
 *   c_solve0()...................Construct right-hand side vector -b- for
 *general boundary conditions STWJ(17) and solve system of eqns. obtained from
 *the b.c.s and the continuity-of-intensity-at-layer-interface eqns.
 *   c_solve1()...................Construct right-hand side vector -b- for
 *isotropic incidence (only) on either top or bottom boundary and solve system
 *of eqns. obtained from the b.c.s and the
 *                                continuity-of-intensity-at-layer-interface
 *eqns. c_twostr_solve_bc()..........Construct right-hand side vector -b- for
 *general b.c. and solve system of eqns. obtained from the b.c.s and the
 *continuity-of-intensity-at-layer-interface eqns.
 *   c_surface_bidir()............Compute user's surface bidirectional
 *properties, STWL(41) (Fortran name surfac).
 *   c_interp_eigenvec()..........Interpolate eigenvectors to user angles; eq
 *SD(8) (Fortran name terpev). c_interp_source()............Interpolate source
 *functions to user angles, eq. STWL(30) (Fortran name terpso).
 *   c_upbeam()...................Find the incident-beam particular solution of
 *SS(18), STWL(24a). c_upisot()...................Find the particular solution
 *of thermal radiation of STWL(25). c_user_intensities().........Compute
 *intensity components at user output angles for azimuthal expansion terms in
 *eq. SD(2), STWL(6) (Fortran name usrint).
 *   c_xi_fun()...................Calculates Xi function of eq. STWL (72)
 *(Fortran name xifunc). c_check_inputs().............Check the input dimensions
 *and variables (Fortran name chekin). c_dref().....................Flux albedo
 *for given angle of incidence, given a bidirectional reflectivity.
 *   c_legendre_poly()............Compute the normalized associated Legendre
 *polynomial, defined in terms of the associated Legendre polynomial (Fortran
 *name lepoly). c_planck_func1().............Compute Planck function integrated
 *between two wavenumbers (Fortran name plkavg).
 *   c_planck_func2().............Compute Planck function integrated between two
 *wavenumbers, or Planck function at a specific wavenumber (Fortran name
 *tplkavg). c_print_avg_intensities()....Print azimuthally averaged intensities
 *at user angles (Fortran name pravin). c_print_inputs().............Print
 *values of input variables (Fortran name prtinp).
 *   c_print_intensities()........Print the intensity at user polar and
 *azimuthal angles (Fortran name prtint). c_gaussian_quadrature()......Compute
 *weights and abscissae for ordinary Gaussian quadrature on the interval (0,1);
 *that is, such that sum(i=1 to M) ( GWT(i) f(GMU(i)) ) is a good approximation
 *to integral(0 to 1) (f(x) dx) (Fortran name qgausn).
 *   c_ratio()....................Returns ratio a/b with overflow and underflow
 *protection, or 1.+a if b == 0. c_self_test()................Sets up self test
 *and compares results (Fortran name slftst).
 *   c_albtrans().................DISORT special case to get only albedo and
 *transmissivity of entire medium as a function of incident beam angle (Fortran
 *name albtrn). c_albtrans_intensity().......Computes azimuthally-averaged
 *intensity at top and bottom of medium (related to albedo and transmission of
 *medium by reciprocity principles; see Ref S2; Fortran name altrin).
 *   c_print_albtrans()...........Print planar albedo and transmissivity of
 *medium as a function of incident beam angle (Fortran name praltr).
 *   c_albtrans_spherical().......Calculate spherical albedo and transmissivity
 *for the entire medium from the m=0 intensity components; a specialized version
 *of fluxes (Fortran name spaltr). c_errmsg()...................Print out a
 *warning or error message; abort if error. c_write_bad_var()............Write
 *names of erroneous variable, keep count (Fortran name wrtbad, wrtbad2).
 *   c_write_too_small_dim()......Write name of too-small symbolic dimension and
 *the value to which it should be increased (Fortran name wrtdim, wrtdim2).
 *   c_sgbco()....................Factor a band matrix by Gaussian elimination
 *and estimate the condition of the matrix. c_sgbfa()....................Factor
 *a band matrix by elimination. c_afval()....................Solve the band
 *system A*X = B or transpose(A)*X = B using the factors computed by sgbco() or
 *sgbfa(). c_sgeco()....................Factor a matrix by Gaussian elimination
 *and estimate the condition of the matrix. c_sgefa()....................Factor
 *a matrix by Gaussian elimination. c_sgesl()....................Solve the
 *system A*X = B or transpose(A)*X = B using the factors computed by sgeco() or
 *sgefa(). c_sasum()....................Sum of absolute values of elements in an
 *array. c_saxpy()....................Compute A*X + Y given scalar A and vectors
 *X and Y. c_sdot().....................Dot product (inner product) of input
 *vectors X and Y. c_sscal()....................Multiply given scalar and
 *vector. c_isamax()...................Return biggest absolute value among the
 *array elements. c_twostr()...................Solve the radiative transfer
 *equation in the two-stream approximation. Based on the general-purpose
 *algorithm DISORT, but both simplified and extended.
 *   c_chapman()..................Calculate the Chapman factor.
 *   c_twostr_check_inputs()......Check input dimensions and variables for two
 *stream code (Fortran name tchekin). c_twostr_fluxes()............Calculate
 *radiative fluxes, mean intensity, and flux derivative with respect to optical
 *depth from the azimuthally-averaged intensity (Fortran name tfluxes).
 *   c_twostr_solns().............Calculates the homogenous and particular
 *solutions to the radiative transfer equation in the two-stream approximation,
 *for each layer in the medium (Fortran name hopsol).
 *   c_twostr_print_inputs()......Print values of input variables (Fortran name
 *tprtinp). c_twostr_set()...............Perform miscellaneous setting-up
 *operations (Fortran name settwo).
 *
 * Functions added to C version:
 *
 *   c_fcmp().....................Safe floating-point comparison function (more
 *reliable than ==) c_disort_state_alloc().......Dynamically allocate memory for
 *disort input arrays, incl. ones the user can ask disort() to calculate
 *   c_disort_state_free()........Free memory allocated by disort_state_alloc()
 *   c_disort_out_alloc_()........Dynamically allocate memory for disort output
 *arrays c_disort_out_free()..........Free memory allocated by
 *disort_out_alloc() c_twostr_state_alloc().......Dynamically allocate memory
 *for twostr input arrays c_twostr_state_free(0........Free memory allocated by
 *twostr_state_alloc() c_twostr_out_alloc().........Dynamically allocate memory
 *for twostr output arrays c_twostr_out_free()..........Free memory allocated by
 *twostr_out_alloc() c_dbl_vector()...............Allocate zeroed memory for
 *double-precision vector of given range c_int_vector()...............Allocate
 *zeroed memory for integer vector of given range
 *   c_free_dbl_vector()..........Free memory allocated by dbl_vector()
 *
 *  C rewrite by Timothy E. Dowling (Univ. of Louisville)
 *  This rewrite includes conversion to double precision, dynamic memory
 *allocation, introduction of C structures (which may yield beneficial
 *cache-aware memory allocation), and improved readability of subroutine names.
 ...
 *-------------------------------------------------------------------------------*
  ...
  Calling Tree (omitting calls to c_errmsg, c_dbl_vector, c_int_vector,
 c_free_dbl_vector):
 ...
 +-------------------------------------------------------------------+
  ...
  iq/2     :  for half the computational polar angles (just the ones in either
 0-90 degrees, or 90-180 degrees) j        :  for user azimuthal angles k,l :
 for Legendre expansion coefficients or, alternatively, subscripts of associated
 Legendre polynomials lu       :  for user levels lc       :  for computational
 layers (each having a different single-scatter albedo and/or phase function)
  lev      :  for computational levels
  mazim    :  for azimuthal components in Fourier cosine expansion of intensity
 and phase function
 ...
 +------------------------------------------------------------------+
   ...
   AMB(iq/2,iq/2)....First  matrix factor in reduced eigenvalue problem of eqs.
 SS(12), STWJ(8E), STWL(23f) (used only in solve_eigen); ab[].zero (see
 cdisort.h) APB(iq/2,iq/2)....Second matrix factor in reduced eigenvalue problem
 of eqs. SS(12), STWJ(8E), STWL(23f) (used only in solve_eigen); ab[].one (see
 cdisort.h) ARRAY(iq,iq)......Scratch matrix for solve_eigen(), upbeam() and
 upisot() (see each subroutine for definition) B()...............Right-hand side
 vector of eq. SC(5) going into SOLVE0,1; returns as solution vector vector  L,
 the constants of integration BDR(iq/2,0:iq/2)..Bottom-boundary bidirectional
 reflectivity for a given azimuthal component.  First index always refers to a
 computational angle.  Second index: if zero, refers to incident beam angle
 UMU0; if non-zero, refers to a computational angle.
   BEM(iq/2).........Bottom-boundary directional emissivity at computational
 angles. bplanck...........Intensity emitted from bottom boundary
   callnum...........Number of surface calls
   CBAND()...........Matrix of left-hand side of the linear system eq. SC(5),
 scaled by eq. SC(12); in banded form required by LINPACK solution routines
   CC(iq,iq).........C-sub-IJ in eq. SS(5)
   CH(lc)............The Chapman-factor to correct for pseudo-spherical geometry
 in the direct beam. CHTAU(lc).........The optical depth in spherical geometry.
   CMU(iq)...........Computational polar angles (Gaussian)
   CWT(iq)...........Quadrature weights corresponding to CMU
   corint............When set TRUE, correct intensities for delta-scaling
 effects (see Nakajima and Tanaka, 1988). When FALSE, intensities are not
 corrected. In general, CORINT should be set true when beam source is present
 (FBEAM is not zero) and DELTAM is TRUE in a problem including scattering.
                     However, execution is faster when CORINT is FALSE, and
 intensities outside the aureole may still be accurate enough.  When CORINT is
 TRUE, it is important to have a sufficiently high order of Legendre
 approximation of the phase function. This is because the intensities are
 corrected by calculating the single-scattered radiation, for which an adequate
 representation of the phase function is crucial.  In case of a low order
 Legendre approximation of an otherwise highly anisotropic phase function, the
 intensities might actually be more accurate when corint is FALSE. When only
 fluxes are calculated (ds->flag.onlyfl is TRUE), or there is no beam source
 (FBEAM=0.0), or there is no scattering (SSALB =0. for all layers) corint is set
 FALSE by the code. delm0.............Kronecker delta, delta-sub-M0, where M =
 MAZIM is the number of the Fourier component in the azimuth cosine expansion
 ...
 fluxes will be more accurate for phase functions with a large forward peak if
 DELTAM is set true. Intensities close to the forward scattering direction are
 often less accurate, however, when the delta-M method is applied. The intensity
 deltam correction of Nakajima and Tanaka is used to improve the accuracy of the
 intensities. dither............Small quantity subtracted from single-scattering
 albedos of unity, in order to avoid using special case formulas;  prevents an
 eigenvalue of exactly zero from occurring, which would cause an immediate
 overflow DTAUCPR(lc).......Computational-layer optical depths (delta-M-scaled
 if DELTAM = TRUE, otherwise equal to DTAUC) EMU(iu)...........Bottom-boundary
 directional emissivity at user angles. EVAL(iq)..........Temporary storage for
 eigenvalues of eq. SS(12) EVECC(iq,iq)......Complete eigenvectors of SS(7) on
 return from solve_eigen; stored permanently in  GC
   EXPBEA(lc)........Transmission of direct beam in delta-M optical depth
 coordinates FLYR(lc)..........Separated fraction in delta-M method
   GL(k,lc)..........Phase function Legendre polynomial expansion coefficients,
 calculated from PMOM by including single-scattering albedo, factor 2K+1, and
 (if DELTAM=TRUE) the delta-M scaling GC(iq,iq,lc)......Eigenvectors at polar
 quadrature angles, g in eq. SC(1) GU(iu,iq,lc)......Eigenvectors interpolated
 to user polar angles (g  in eqs. SC(3) and S1(8-9), i.e. g without the l
 factor) IPVT(lc*iq).......Integer vector of pivot indices for LINPACK routines
   KK(iq,lc).........Eigenvalues of coeff. matrix in eq. SS(7)
   kconv.............Counter in azimuth convergence test
   LAYRU(lu).........Computational layer in which user output level UTAU(LU) is
 located LL(iq,lc).........Constants of integration L in eq. SC(1), obtained by
 solving scaled version of eq. SC(5) lyrcut............TRUE, radiation is
 assumed zero below layer ncut because of almost complete absorption
   naz...............Number of azimuthal components considered
   ncut..............Computational layer number in which absorption optical
 depth first exceeds ABSCUT OPRIM(lc).........Single scattering albedo after
 delta-M scaling pass1.............TRUE on first entry, FALSE thereafter
   PKAG(0:lc)........Integrated Planck function for internal emission
   PRNTU0(l).........logical flag to trigger printing of azimuthally-averaged
 intensities: l    quantities printed
                      --    ------------------
   ...
   PSI0(iq)..........Sum just after square bracket in  eq. SD(9); psi[].zero
 (see cdisort.h) PSI1(iq)..........Sum in  eq. STWL(31d); psi[].one
   RMU(iu,0:iq)......Bottom-boundary bidirectional reflectivity for a given
 azimuthal component.  First index always refers to a user angle.  Second index:
 if zero, refers to incident beam angle UMU0; if non-zero, refers to a
 computational angle. scat_yes..........int, TRUE if scattering, FALSE if not
 (added to C version) TAUC(0:lc)........Cumulative optical depth
 (un-delta-M-scaled) TAUCPR(0:lc)......Cumulative optical depth (delta-M-scaled
 if DELTAM = TRUE, otherwise equal to TAUC) tplanck...........Intensity emitted
 from top boundary UUM(iu,lu)........Expansion coefficients when the intensity
 (u-super-M) is expanded in Fourier cosine series in azimuth angle
   U0C(iq,lu)........Azimuthally-averaged intensity at quadrature angle
   U0U(iu,lu)........If ds->flag.onlyfl = FALSE, azimuthally-averaged intensity
 at user angles and user levels If ds->flag.onlyfl = TRUE, azimuthally-averaged
 intensity at computational (Gaussian quadrature) angles and user levels; the
 corresponding quadrature angle cosines are returned in UMU.
   UTAUPR(lu)........Optical depths of user output levels in delta-M
 coordinates; equal to UTAU(LU) if no delta-M WK(iq)............Scratch array
   XR0(lc)...........X-sub-zero in expansion of thermal source function
 preceding eq. SS(14)(has no mu-dependence); b-sub-zero in eq. STWL(24d)
   XR1(lc)...........X-sub-one in expansion of thermal source function; see eqs.
 SS(14-16); b-sub-one in STWL(24d) YLM0(l)...........Normalized associated
 Legendre polynomial of subscript L at the beam angle (not saved as function of
 superscipt M) YLMC(l,iq)........Normalized associated Legendre polynomial of
 subscript L at the computational angles (not saved as function of superscipt M)
   YLMU(l,iu)........Normalized associated Legendre polynomial of subscript L at
 the user angles (not saved as function of superscipt M)
   Z()...............scratch array used in solve0(), albtrans() to solve a
 linear system for the constants of integration Z0(iq)............Solution
 vectors Z-sub-zero of eq. SS(16); zee[].zero (see cdisort.h)
   Z1(iq)............Solution vectors Z-sub-one  of eq. SS(16); zee[].one
   Z0U(iu,lc)........Z-sub-zero in eq. SS(16) interpolated to user angles from
 an equation derived from SS(16); zu[].zero (see cdisort.h)
   Z1U(iu,lc)........Z-sub-one  in eq. SS(16) interpolated to user angles from
 an equation derived from SS(16); zu[].one ZBEAM(iu,lc)......Particular solution
 for beam source ZGU(iu,lc)........General source function interpolated to user
 angles ZJ(iq)............Right-hand side vector  X-sub-zero in eq. SS(19), also
 the solution vector Z-sub-zero after solving that system
   ZJG(iq)...........Right-hand side vector  X-sub-zero in eq. KS(10), also the
 solution vector Z-sub-zero after solving that system for a general source
 constant over a layer ZZ(iq,lc).........Permanent storage for the beam source
 vectors ZJ ZZG(iq,lc)........Permanent storage for the beam source vectors ZJG
   ZPLK0(iq,lc)......Permanent storage for the thermal source vectors plk[].zero
 obtained by solving eq. SS(16) ZPLK1(iq,lc)......Permanent storage for the
 thermal source vectors plk[].one  obtained by solving eq. SS(16)

*/

int c_disort(disort_state *ds, disort_output *out) {
  static int self_tested = -1;
  int prntu0[2], corint, deltam, scat_yes, compare, lyrcut, needdeltam, iq, iu,
      j, kconv, l, lc, lev, lu, mazim, naz, ncol, ncos, ncut, nn;
  static int callnum = 1;
  int ipvt[ds->nstr * ds->nlyr], layru[ds->ntau];
  double angcos, azerr, azterm, bplanck, cosphi, delm0, sgn, tplanck;
  double *array, *b, *bdr, *bem, *cband, *cc, *ch, *chtau, *cmu, *cwt, *dtaucpr,
      *emu, *eval, *evecc, *expbea, *flyr, *gc, *gl, *gu, *kk, *ll, *oprim,
      *phasa, *phast, *phasm, *phirad, *pkag, *rmu, *tauc, *taucpr, *u0c,
      *utaupr, *uum, *wk, *xba, *ylm0, *ylmc, *ylmu, *z, *zbeam, *zbeama,
      zbsa = 0, *zj, *zjg, *zju, *zgu, *zz, *zzg;
  ...
  if (deltam == TRUE) {
    for (lc = 1; lc <= ds->nlyr; lc++)
      if (PMOM(ds->nstr, lc) != 0.0) needdeltam = TRUE;
    if (needdeltam == FALSE) deltam = FALSE;
  }
  ...
  /*-------------------------------------------------------------------------------------------*
   * Special case for getting albedo and transmissivity of medium for many beam
   *angles at once *
   *-------------------------------------------------------------------------------------------*/
  ...
  if (ds->flag.ibcnd == SPECIAL_BC) {
    ...
    cband = c_dbl_vector(0, ds->nstr * ds->nlyr * (9 * (ds->nstr / 2) - 2) - 1,
                         "cband");
    ...
    ab = (disort_pair *)calloc((ds->nstr / 2) * (ds->nstr / 2),
                               sizeof(disort_pair));
    if (!ab) {
    ...
    if (ds->flag.prnt[0]) {
      c_print_inputs(ds, dtaucpr, scat_yes, deltam, corint, flyr, lyrcut, oprim,
                     tauc, taucpr);
    }

    c_albtrans(ds, out, ab, array, b, bdr, cband, cc, cmu, cwt, dtaucpr, eval,
               evecc, gl, gc, gu, ipvt, kk, ll, nn, taucpr, ylmc, ylmu, z, wk);
  ...
  /*
   * Using C structures facilitates cache-aware memory allocation, which can
   * reduce cache misses and potentially speed up computer execution.
   */
  ...
  if (!plk) c_errmsg("disort alloc error for plk", DS_ERROR);
  ab = (disort_pair *)calloc((ds->nstr / 2) * (ds->nstr / 2),
                             sizeof(disort_pair));
  if (!ab) c_errmsg("disort alloc error for ab", DS_ERROR);
  xr = (disort_pair *)calloc(ds->nlyr, sizeof(disort_pair));
  if (!xr) c_errmsg("disort alloc error for xr", DS_ERROR);
  psi = (disort_pair *)calloc(ds->nstr, sizeof(disort_pair));
  if (!psi) c_errmsg("disort alloc error for psi", DS_ERROR);
  xb = (disort_pair *)calloc(ds->nlyr * ds->nstr, sizeof(disort_pair));
  if (!xb) c_errmsg("disort alloc error for xb", DS_ERROR);
  zbs = (disort_pair *)calloc(ds->nstr, sizeof(disort_pair));
  if (!zbs) c_errmsg("disort alloc error for zbs", DS_ERROR);
  zbeamsp = (disort_pair *)calloc(ds->nlyr * ds->nstr, sizeof(disort_pair));
  if (!zbeamsp) c_errmsg("disort alloc error for zbeamsp", DS_ERROR);
  zee = (disort_pair *)calloc(ds->nstr, sizeof(disort_pair));
  if (!zee) c_errmsg("disort alloc error for zee", DS_ERROR);
  zu = (disort_pair *)calloc(ds->nlyr * ds->numu, sizeof(disort_pair));
  if (!zu) c_errmsg("disort alloc error for zu", DS_ERROR);

  zbu = (disort_triplet *)calloc(ds->nlyr * ds->numu, sizeof(disort_triplet));
  if (!zbu) c_errmsg("disort alloc error for zbu", DS_ERROR);
  ...
  if (!ds->flag.planck) {
    ...
    tplanck =
        c_planck_func2(ds->wvnmlo, ds->wvnmhi, ds->bc.ttemp) * ds->bc.temis;
   ...
   */
  if (ds->bc.fbeam == 0. || fabs(1. - ds->bc.umu0) < 1.e-5 || ds->flag.onlyfl ||
      (ds->numu == 1 && fabs(1. - UMU(1)) < 1.e-5) ||
      (ds->numu == 1 && fabs(1. + UMU(1)) < 1.e-5) ||
      (ds->numu == 2 && fabs(1. + UMU(1)) < 1.e-5 &&
       fabs(1. - UMU(2)) < 1.e-5)) {
    naz = 0;
  }
  ...
  for (mazim = 0; mazim <= naz; mazim++) {
    ...
    /*
     * Get normalized associated Legendre polynomials with negative arguments
     * from those with positive arguments; Dave/Armstrong eq. (15), STWL(59)
     */
    sgn = -1.;
    ...
    for (lc = 1; lc <= ncut; lc++) {
       ...
       */
      c_solve_eigen(ds, lc, ab, array, cmu, cwt, gl, mazim, nn, ylmc, cc, evecc,
                    eval, kk, gc, wk);
      /*
       * Calculate particular solutions of eq. SS(18), STWL(24a) for incident
       * beam source
       */
      if (ds->bc.fbeam > 0.) {
        if (ds->flag.spher == TRUE) {
          /* Pseudo-spherical approach */
          c_set_coefficients_beam_source(ds, ch, chtau, cmu, delm0,
                                         ds->bc.fbeam, gl, lc, mazim, ds->nstr,
                                         taucpr, xba, xb, ylm0, ylmc, zj);

          if (ds->flag.usrang == TRUE) {
            /* Get coefficients at umu for pseudo-spherical source */
            c_interp_coefficients_beam_source(
                ds, chtau, delm0, ds->bc.fbeam, gl, lc, mazim, ds->nstr,
                ds->numu, taucpr, zbu, xba, zju, ylm0, ylmu);
          }
          c_upbeam_pseudo_spherical(ds, lc, array, cc, cmu, ipvt, nn, wk, xb,
                                    xba, zbs, &zbsa, zbeamsp, zbeama);
        } else {
          /* Plane-parallel version */
          c_upbeam(ds, lc, array, cc, cmu, delm0, gl, ipvt, mazim, nn, wk, ylm0,
                   ylmc, zj, zz);
        }
      }
      ...
      if (ds->flag.general_source) {
        c_upbeam_general_source(ds, lc, mazim, array, cc, ipvt, nn, wk, zjg,
                                zzg);
      }

      /*
       * Calculate particular solutions of eq. SS(15), STWL(25) for thermal
       * emission source
       */
      if (ds->flag.planck && mazim == 0) {
        XR1(lc) = 0.;
        if (DTAUCPR(lc) >
            1e-4) { /* fix by RPB, caused problems in make check AVHRR CH4/5 */
      ...
      if (!ds->flag.onlyfl && ds->flag.usrang) {
         ...
         */
        c_interp_eigenvec(ds, lc, cwt, evecc, gl, gu, mazim, nn, wk, ylmc,
                          ylmu);
        /*
    ...
    c_solve0(ds, b, bdr, bem, bplanck, cband, cmu, cwt, expbea, ipvt, ll,
             lyrcut, mazim, ncol, ncut, nn, tplanck, taucpr, z, zbeamsp, zbeama,
             zz, zzg, plk);
     ...
     */
    if (mazim == 0) {
      c_fluxes(ds, out, ch, cmu, cwt, gc, kk, layru, ll, lyrcut, ncut, nn,
               PRNTU0(1), taucpr, utaupr, xr, zbeamsp, zbeama, zz, zzg, plk, fl,
               u0c);
    }
    ...
    if (ds->flag.usrang) {
       ...
       */
      c_user_intensities(ds, bplanck, cmu, cwt, delm0, dtaucpr, emu, expbea, gc,
                         gu, kk, layru, ll, lyrcut, mazim, ncut, nn, rmu,
                         taucpr, tplanck, utaupr, wk, zbu, zbeam, zbeamsp,
                         zbeama, zgu, zu, zz, zzg, plk, uum);
    } else {
      /*
       * Compute azimuthal intensity components at quadrature angles
       */
      c_intensity_components(ds, gc, kk, layru, ll, lyrcut, mazim, ncut, nn,
                             taucpr, utaupr, zz, plk, uum);
    }
  ...
  if (corint) {
    ...
    if (!ds->flag.old_intensity_correction && self_tested == 1) {
      ...
      c_new_intensity_correction(ds, out, dither, flyr, layru, lyrcut, ncut,
                                 oprim, phasa, phast, phasm, phirad, tauc,
                                 taucpr, utaupr);
    } else {
      if (ds->flag.quiet == VERBOSE)
        fprintf(stderr,
                "Using original intensity correction, with phase moments\n");
      c_intensity_correction(ds, out, dither, flyr, layru, lyrcut, ncut, oprim,
                             phasa, phast, phasm, phirad, tauc, taucpr, utaupr);
    }
  }
  ...
  free(ll);
  free(oprim);
  free(phasa), free(phast), free(phasm), free(phirad), free(pkag), free(plk),
      free(psi);
  free(rmu);
  ...
  free(z), free(zbu), free(zbeam), free(zbeamsp), free(zbeama), free(zbs),
      free(zj), free(zjg), free(zju), free(zgu), free(zz), free(zzg), free(zee),
      free(zu);
...
/*
  ...
  NOTE 2: Both MU and MU0 (cosines of reflection and incidence angles) are
positive.
...
double c_bidir_reflectivity(double wvnmlo, double wvnmhi, double mu, double mup,
                            double dphi, int brdf_type, disort_brdf *brdf,
                            int callnum) {
  int irmu;

  double ans, rmu, flxalb;

  static double badmu, swvnmlo, swvnmhi, srho0, sk, stheta, ssigma, st1, st2,
      sscale;

#if HAVE_BRDF
  static double siso, svol, sgeo;
#endif
  ...
  switch (brdf_type) {
    case BRDF_HAPKE:
      ...
      break;
    case BRDF_RPV:
      if (swvnmlo != wvnmlo || swvnmhi != wvnmhi || srho0 != brdf->rpv->rho0 ||
          sk != brdf->rpv->k || stheta != brdf->rpv->theta ||
          ssigma != brdf->rpv->sigma || st1 != brdf->rpv->t1 ||
          st2 != brdf->rpv->t2 || sscale != brdf->rpv->scale) {
        swvnmlo = wvnmlo;
        swvnmhi = wvnmhi;
        srho0 = brdf->rpv->rho0;
        sk = brdf->rpv->k;
    ...
    case BRDF_CAM:
      ...
      /*
       * NOTE: Should group brdf->cam input arguments into the single pointer
       * brdf->cam, in the same manner as brdf->rpv for
       * c_bidir_reflectivity_rpv().
       */
      ans = ocean_brdf(wvnmlo, wvnmhi, mu, mup, dphi, brdf->cam->u10,
                       brdf->cam->pcl, brdf->cam->xsal, callnum);
      ...
      if (ans != ans) {
        fprintf(stderr,
                ...
                wvnmlo, wvnmhi, mu, mup, dphi, brdf->cam->u10, brdf->cam->pcl,
                brdf->cam->xsal);
        ans = 1.0;
      }
#else
    ...
    case BRDF_AMB:
      ...
      if (siso != brdf->ambrals->iso || svol != brdf->ambrals->vol ||
      ...
      if (badmu > 0.0) {
        if (mu < badmu) mu = badmu;
        if (mup < badmu) mup = badmu;
      }

      /*
       * NOTE: Should group brdf->ambrals input arguments into the single
       * pointer brdf->ambrals, in the same manner as brdf->rpv for
       * c_bidir_reflectivity_rpv().
       */
      ans = ambrals_brdf(brdf->ambrals->iso, brdf->ambrals->vol,
                         brdf->ambrals->geo, mu, mup, dphi);
      ...
      if (ans != ans) {
        fprintf(stderr,
                "NaN returned from ambrals_brdf: %e %e %e %e %e %e %e %e\n",
                wvnmlo, wvnmhi, mu, mup, dphi, brdf->ambrals->iso,
                brdf->ambrals->vol, brdf->ambrals->geo);
        ans = 1.0;
      }
#else
 ...
 * eq. 8.89 on page 233. Parameters are from Fig. 8.15 on page 231, except for
w.
  ...
  LOCAL VARIABLES:
    ...
    b0     : empirical factor to account for the finite size of particles in
Hapke's BDR model b      : term that accounts for the opposition effect
(retroreflectance, hot spot) in Hapke's BDR model ctheta : cosine of phase angle
in Hapke's BDR model gamma  : albedo factor in Hapke's BDR model h0     : H(mu0)
in Hapke's BDR model h      : H(mu) in Hapke's BDR model hh     : angular width
parameter of opposition effect in Hapke's BDR model p      : scattering phase
function in Hapke's BDR model theta  : phase angle (radians); the angle between
incidence and reflection directions in Hapke's BDR model w      : single
scattering albedo in Hapke's BDR model

   Called by- c_bidir_reflectivity
-------------------------------------------------------------------------*/

double c_bidir_reflectivity_hapke(double wvnmlo, double wvnmhi, double mu,
                                  double mup, double dphi) {
  double b0, b, ctheta, Xgamm, h0, h, hh, p, thetah, w;
...
double c_bidir_reflectivity_rpv(rpv_brdf_spec *brdf, double mu1, double mu2,
                                double phi, double badmu) {
  ...
  /* Hot spot */
  hspot = brdf->rho0 *
          (pow(2.0 * mu1 * mu1 * mu1, brdf->k - 1.0) * (1.0 - brdf->theta) /
               (1.0 + brdf->theta) / (1.0 + brdf->theta) * (2.0 - brdf->rho0) +
           brdf->sigma / mu1) *
          (brdf->t1 * exp(M_PI * brdf->t2) + 1.0);

  /* Hot spot region */
  /* is this bug??? phi <= 1e-4 would be more sensible ... RPB */
  if (phi == 1e-4 && mu1 == mu2) return hspot * brdf->scale;
...
/*
 * Calculate phase function Legendre expansion coefficients in various special
 * cases.
 *
 *  INPUT
 ...
 *    nmom    Index of highest Legendre coefficient needed (number of streams
 * 'nstr' chosen for the discrete ordinate method). Set to -1 for no scattering.
 *  OUTPUT
 *    PMOM(k) Legendre expansion coefficients (k = 0 to nmom)
 *
 * Reference:  Garcia, R. and C. Siewert, 1985: Benchmark Results in Radiative
 * Transfer, Transp. Theory and Stat. Physics 14, 437-484, Tables 10 And 17
 */

void c_getmom(int iphas, double gg, int nmom, double *pmom) {
  const double cldmom[299] = {
      2.544,  3.883,  4.568,  5.235,  5.887,  6.457,  7.177,  7.859,  8.494,
      9.286,  9.856,  10.615, 11.229, 11.851, 12.503, 13.058, 13.626, 14.209,
      14.660, 15.231, 15.641, 16.126, 16.539, 16.934, 17.325, 17.673, 17.999,
      18.329, 18.588, 18.885, 19.103, 19.345, 19.537, 19.721, 19.884, 20.024,
      20.145, 20.251, 20.330, 20.401, 20.444, 20.477, 20.489, 20.483, 20.467,
      20.427, 20.382, 20.310, 20.236, 20.136, 20.036, 19.909, 19.785, 19.632,
      19.486, 19.311, 19.145, 18.949, 18.764, 18.551, 18.348, 18.119, 17.901,
      17.659, 17.428, 17.174, 16.931, 16.668, 16.415, 16.144, 15.883, 15.606,
      15.338, 15.058, 14.784, 14.501, 14.225, 13.941, 13.662, 13.378, 13.098,
      12.816, 12.536, 12.257, 11.978, 11.703, 11.427, 11.156, 10.884, 10.618,
      10.350, 10.090, 9.827,  9.574,  9.318,  9.072,  8.822,  8.584,  8.340,
      8.110,  7.874,  7.652,  7.424,  7.211,  6.990,  6.785,  6.573,  6.377,
      6.173,  5.986,  5.790,  5.612,  5.424,  5.255,  5.075,  4.915,  4.744,
      4.592,  4.429,  4.285,  4.130,  3.994,  3.847,  3.719,  3.580,  3.459,
      3.327,  3.214,  3.090,  2.983,  2.866,  2.766,  2.656,  2.562,  2.459,
      2.372,  2.274,  2.193,  2.102,  2.025,  1.940,  1.869,  1.790,  1.723,
      1.649,  1.588,  1.518,  1.461,  1.397,  1.344,  1.284,  1.235,  1.179,
      1.134,  1.082,  1.040,  0.992,  0.954,  0.909,  0.873,  0.832,  0.799,
      0.762,  0.731,  0.696,  0.668,  0.636,  0.610,  0.581,  0.557,  0.530,
      0.508,  0.483,  0.463,  0.440,  0.422,  0.401,  0.384,  0.364,  0.349,
      0.331,  0.317,  0.301,  0.288,  0.273,  0.262,  0.248,  0.238,  0.225,
      0.215,  0.204,  0.195,  0.185,  0.177,  0.167,  0.160,  0.151,  0.145,
      0.137,  0.131,  0.124,  0.118,  0.112,  0.107,  0.101,  0.097,  0.091,
      0.087,  0.082,  0.079,  0.074,  0.071,  0.067,  0.064,  0.060,  0.057,
      0.054,  0.052,  0.049,  0.047,  0.044,  0.042,  0.039,  0.038,  0.035,
      0.034,  0.032,  0.030,  0.029,  0.027,  0.026,  0.024,  0.023,  0.022,
      0.021,  0.020,  0.018,  0.018,  0.017,  0.016,  0.015,  0.014,  0.013,
      0.013,  0.012,  0.011,  0.011,  0.010,  0.009,  0.009,  0.008,  0.008,
      0.008,  0.007,  0.007,  0.006,  0.006,  0.006,  0.005,  0.005,  0.005,
      0.005,  0.004,  0.004,  0.004,  0.004,  0.003,  0.003,  0.003,  0.003,
...
/*
  ...
  Wilkinson, J., 1965: The Algebraic Eigenvalue Problem, Clarendon Press,
   ...
   O U T P U T    V A R I A B L E S:
       ...
       evec  :  (unnormalized) eigenvectors of aa (column j corresponds to
 EVAL(J)) eval  :  (unordered) eigenvalues of aa (dimension m) ier   :  if != 0,
 signals that EVAL(ier) failed to converge; in that case eigenvalues
 ier+1,ier+2,...,m  are correct but eigenvalues 1,...,ier are set to zero.
...
void c_asymmetric_matrix(double *aa, double *evec, double *eval, int m, int ia,
                         int ievec, int *ier, double *wk) {
  const double c1 = .4375, c2 = .5, c3 = .75, c4 = .95, c5 = 16., c6 = 256.;
  int noconv, notlas, i, ii, in, j, k, ka, kkk, l, lb = 0, lll, n, n1, n2;
  double col, discri, f, g, h, p = 0, q = 0, r = 0, repl, rnorm, row, s, scale,
                               sgn, t, tol, uu, vv, w, x, y, z;
  ...
  *ier = 0;
  ...
  /*
   * Balance the input matrix and reduce its norm by diagonal similarity
   * transformation stored in wk; then search for rows isolating an eigenvalue
   * and push them down.
   */
  rnorm = 0.;
  l = 1;
...
/*
 ...
 scaled version of eq. SC(5); exponential term of eq. SC(12) not included lyrcut
 :  Logical flag for truncation of computational layer mazim   :  Order of
 azimuthal component ncut    :  Number of computational layer where absorption
 optical depth exceeds ABSCUT nn      :  Order of double-Gauss quadrature
 (NSTR/2) taucpr  :  Cumulative optical depth (delta-M-scaled) utaupr  : Optical
 depths of user output levels in delta-M coordinates;  equal to UTAU if no
 delta-M zz      :  Beam source vectors in eq. SS(19), STWL(24b) plk     :
 Thermal source vectors z0,z1 by solving eq. SS(16), Y-sub-zero, Y-sub-one in
 STWL(26ab); plk[].zero, plk[].one (see cdisort.h)

    O U T P U T   V A R I A B L E S:

       uum     :  Fourier components of the intensity in eq. SD(12) (at polar
 quadrature angles)
...
void c_intensity_components(disort_state *ds, double *gc, double *kk,
                            int *layru, double *ll, int lyrcut, int mazim,
                            int ncut, int nn, double *taucpr, double *utaupr,
                            double *zz, disort_pair *plk, double *uum) {
...
/*
    ...
    I N P U T    V A R I A B L E S:
       ...
       ll       :  Constants of integration in eq. SC(1), obtained by solving
 scaled version of eq. SC(5); exponential term of eq. SC(12) not included lyrcut
 :  Logical flag for truncation of comput. layer ncut     :  Number of
 computational layer where absorption optical depth exceeds ABSCUT nn       :
 Order of double-Gauss quadrature (NSTR/2) prntu0   :  TRUE, print
 azimuthally-averaged intensity at quadrature angles taucpr   :  Cumulative
 optical depth (delta-M-scaled) utaupr   :  Optical depths of user output levels
 in delta-M coordinates;  equal to UTAU if no delta-M xr       :  Expansion of
 thermal source function in eq. SS(14,16), STWL(24c); xr[].zero, xr[].one (see
 cdisort.h) zz       :  Beam source vectors in eq. SS(19), STWL(24b) zzg      :
 Beam source vectors in eq. KS(10)for a general source constant over a layer plk
 :  Thermal source vectors z0,z1 by solving eq. SS(16), Y0,Y1 in STWL(26b,a);
    ...
    I N T E R N A L    V A R I A B L E S:
       ...
       fl       :  fl[].zero: 'fldir' = direct-beam flux (delta-M scaled),
 fl[].one 'fldn' = diffuse down-flux (delta-M scaled) fnet     :  Net flux
 (total_down-diffuse_up) fact     :  EXP(- UTAUPR/UMU0) plsorc   :  Planck
 source function (thermal) zint     :  Intensity of m = 0 case, in eq. SC(1)

   Called by- c_disort
 -------------------------------------------------------------------*/

void c_fluxes(disort_state *ds, disort_output *out, double *ch, double *cmu,
              double *cwt, double *gc, double *kk, int *layru, double *ll,
              int lyrcut, int ncut, int nn, int prntu0, double *taucpr,
              double *utaupr, disort_pair *xr, disort_pair *zbeamsp,
              double *zbeama, double *zz, double *zzg, disort_pair *plk,
              disort_pair *fl, double *u0c) {
  register int iq, jq, lu, lyu;
  ...
  if (ds->flag.prnt[1]) {
    fprintf(stdout,
            "\n\n                     <----------------------- FLUXES "
            "----------------------->\n"
            "   Optical  Compu    Downward    Downward    Downward      Upward "
            "                   Mean      Planck   d(Net Flux)\n"
            "     Depth  Layer      Direct     Diffuse       Total     Diffuse "
            "        Net   Intensity      Source   / d(Op Dep)\n");
  }
  ...
  for (lu = 1; lu <= ds->ntau; lu++) {
    ...
    if (lyrcut && lyu > ncut) {
      ...
      if (ds->flag.prnt[1]) {
        fprintf(stdout,
                ...
                UTAU(lu), lyu, RFLDIR(lu), RFLDN(lu), fdntot, FLUP(lu), fnet,
    ...
    if (ds->bc.fbeam > 0.) {
      if (ds->flag.spher == TRUE) {
        ...
        RFLDIR(lu) =
            fabs(ds->bc.umu0) * ds->bc.fbeam * exp(-UTAU(lu) / CH(lyu));
      } else {
  ...
  if (prntu0) {
    fprintf(stdout, "\n\n%s\n",
            " ******** AZIMUTHALLY AVERAGED INTENSITIES ( at polar quadrature "
            "angles ) *******");
    for (lu = 1; lu <= ds->ntau; lu++) {
      fprintf(stdout, "\n%s%10.4f\n\n%s\n", " Optical depth =", UTAU(lu),
              "     Angle (deg)   cos(Angle)     Intensity     Angle (deg)   "
              "cos(Angle)     Intensity");
      for (iq = 1; iq <= nn; iq++) {
        ang1 = acos(CMU(2 * nn - iq + 1)) / DEG;
        ang2 = acos(CMU(iq)) / DEG;
        fprintf(stdout, "%16.4f%13.5f%14.3e%16.4f%13.5f%14.3e\n", ang1,
                CMU(2 * nn - iq + 1), U0C(iq, lu), ang2, CMU(iq),
                U0C(iq + nn, lu));
      }
    }
  }
...
void c_intensity_correction(disort_state *ds, disort_output *out, double dither,
                            double *flyr, int *layru, int lyrcut, int ncut,
                            double *oprim, double *phasa, double *phast,
                            double *phasm, double *phirad, double *tauc,
                            double *taucpr, double *utaupr) {
  register int iu, jp, k, lc, ltau, lu;
  double ctheta, dtheta, duims, pl, plm1, plm2, theta0 = 0, thetap = 0, ussndm,
                                                ussp;
  ...
  for (iu = 1; iu <= ds->numu; iu++) {
    ...
    for (jp = 1; jp <= ds->nphi; jp++) {
      ...
      ctheta =
          -ds->bc.umu0 * UMU(iu) +
          sqrt((1. - SQR(ds->bc.umu0)) * (1. - SQR(UMU(iu)))) * cos(PHIRAD(jp));
      /*
      ...
      for (k = 1; k <= ds->nmom; k++) {
        ...
        if (k <= ds->nstr - 1) {
          for (lc = 1; lc <= ncut; lc++) {
            PHASM(lc) += (double)(2 * k + 1) * pl * (PMOM(k, lc) - FLYR(lc)) /
                         (1. - FLYR(lc));
          }
        }
      }
      ...
      for (lu = 1; lu <= ds->ntau; lu++) {
        if (!lyrcut || LAYRU(lu) < ncut) {
          ussndm =
              c_single_scat(dither, LAYRU(lu), ncut, phast, ds->ssalb, taucpr,
                            UMU(iu), ds->bc.umu0, UTAUPR(lu), ds->bc.fbeam);
          ussp = c_single_scat(dither, LAYRU(lu), ncut, phasm, oprim, taucpr,
                               UMU(iu), ds->bc.umu0, UTAUPR(lu), ds->bc.fbeam);
      ...
      if (UMU(iu) < 0. && fabs(theta0 - thetap) <= dtheta) {
         ...
         * Apply IMS method for correction of secondary scattering below top
         * level.
         */
        ltau = 1;
        if (UTAU(1) <= dither) {
...
/*
   Calculates secondary scattered intensity of eq. STWL (A7)
        ...
        stau    sum of layer optical depths between top of atmopshere and layer
 layru
...
double c_secondary_scat(disort_state *ds, int iu, int lu, double ctheta,
                        double *flyr, int layru, double *tauc) {
  register int k, lyr;
  ...
  /*
   * Calculate vertically averaged value of single scattering albedo and
   * separated fraction f, eq. STWL (A.15)
   */
   ...
   */
  return ds->bc.fbeam / (4. * M_PI) * SQR(fbar * wbar) / (1. - fbar * wbar) *
         pspike * c_xi_func(-UMU(iu), umu0p, UTAU(lu));
}
...
void c_new_intensity_correction(disort_state *ds, disort_output *out,
                                double dither, double *flyr, int *layru,
                                int lyrcut, int ncut, double *oprim,
                                double *phasa, double *phast, double *phasm,
                                double *phirad, double *tauc, double *taucpr,
                                double *utaupr) {
  register int iu, jp, k, lc, ltau, lu;
  double ctheta, dtheta, duims, pl, plm1, plm2, theta0 = 0, thetap = 0, ussndm,
                                                ussp;
  ...
  if (need_secondary_scattering == TRUE) {
    ...
    for (it = 1; it <= ds->nphase; it++) {
      ...
      for (lc = 1; lc <= ds->nlyr; lc++) PHASR(lc) = 1.0 - FLYR(lc);
      ...
      for (lu = 1; lu <= ds->ntau; lu++) {
        ...
        for (lyr = 1; lyr <= LAYRU(lu) - 1; lyr++)
          PHAS2(it, lu) +=
              (DSPHASE(it, lyr) - PHASR(lyr)) * SSALB(lyr) * DTAUC(lyr);

        lyr = LAYRU(lu);
        PHAS2(it, lu) += (DSPHASE(it, lyr) - PHASR(lyr)) * SSALB(lyr) *
                         (UTAU(lu) - TAUC(lyr - 1));
      }
    ...
    for (lu = 1; lu <= ds->ntau; lu++) {
      ...
      if (fbar <= tiny || ds->bc.fbeam <= tiny)
        for (it = 1; it <= ds->nphase; it++) PHAS2(it, lu) = 0.0;
      else {
        fbar = 1. / fbar;
        for (it = 1; it <= ds->nphase; it++) PHAS2(it, lu) *= fbar;
      }
      ...
      for (it = 2; it <= ds->nphase; it++)
        f_phas2 += (ds->MUP(it) - ds->MUP(it - 1)) * 0.5 *
                   (PHAS2(it, lu) + PHAS2(it - 1, lu));

      if (f_phas2 != 0.0) {
        norm = 2.0 / f_phas2;
        for (it = 1; it <= ds->nphase; it++) PHAS2(it, lu) *= norm;
      }

    } /* end for lu<ntau */

    prep_double_scat_integr(ds->nphase, ds->ntau, nf, ds->mu_phase, phas2,
    ...
    for (jp = 1; jp <= ds->nphi; jp++) {
      ...
      for (lc = 1; lc <= ncut; lc++)
        PHASA(lc) = DSPHASE(it, lc) +
                    (ctheta - ds->MUP(it)) / (ds->MUP(it + 1) - ds->MUP(it)) *
        ...
        for (lu = ltau; lu <= ds->ntau; lu++) {
          if (!lyrcut || LAYRU(lu) < ncut) {
            duims = c_new_secondary_scat(ds, iu, lu, it, ctheta, flyr,
                                         LAYRU(lu), tauc, nf, phas2, mu_eq,
                                         neg_phas, NORM_PHAS(lu));
            UU(iu, lu, jp) -= duims;
          }
        }
      }
...
void prep_double_scat_integr(int nphase, int ntau, int nf, double *mu_phase,
                             double *phas2, double *mu_eq, int *neg_phas,
                             double *norm_phas) {
  int it = 0, i = 0, lu = 0;
  ...
  for (lu = 1; lu <= ntau; lu++) {
    ...
    for (i = 2; i <= nf - 1; i++) {
      ...
      while (F_PHAS2_ABS(it + 1) < f_phas2) it++;
...
/*
   Calculates secondary scattered intensity, new method (see BDE)
        ...
        dtau    layer optical depth
        stau    sum of layer optical depths between top of atmopshere and layer
 layru umu0p nphase  number of angles for which original phase function
                   (ds->phase) is defined
...
double c_new_secondary_scat(disort_state *ds, int iu, int lu, int it,
                            double ctheta, double *flyr, int layru,
                            double *tauc, int nf, double *phas2, double *mu_eq,
                            int *neg_phas, double norm_phas) {
  register int lyr;
  const double tiny = 1.e-4;
  ...
  pspike2 = calc_phase_squared(ds->nphase, lu, ctheta, nf, ds->mu_phase, phas2,
                               mu_eq, neg_phas, norm_phas);
...
double calc_phase_squared(int nphase, int lu, double ctheta, int nf,
                          double *mu_phase, double *phas2, double *mu_eq,
                          int *neg_phas, double norm_phas) {
  int j = 0, k = 0, it = 0;
  ...
  for (j = 1; j <= nf; j++) {
    ...
    if (ctheta == 1.0 || MU_EQ(j, lu) == 1.0) {
      ...
      phint = M_PI * (PHAS2(it, lu) + (MU_EQ(j, lu) * ctheta - MUP(it)) /
                                          (MUP(it + 1) - MUP(it)) *
                                          (PHAS2(it + 1, lu) - PHAS2(it, lu)));
      if (ctheta == 1.0) phint /= 2.0;
    } else {
      phint = 0.0;
      ...
      if (mumin < mumax) {
        ...
        for (k = imin + 1; k <= imax; k++) {
          ...
          phint +=
              (Dp - D) *
                  sqrt(1.0 - ctheta * ctheta - MU_EQ(j, lu) * MU_EQ(j, lu) +
                       2.0 * ctheta * MU_EQ(j, lu) * MUP(k) - MUP(k) * MUP(k)) +
              ((Dp - D) * ctheta * MU_EQ(j, lu) + Cp - C) *
                  asin((ctheta * MU_EQ(j, lu) - MUP(k)) / (smueq * stheta));
        ...
        if (cutting == TRUE)
          phint +=
              ...
              (D * ctheta * MU_EQ(j, lu) + C) *
                  asin((ctheta - 1.0) * MU_EQ(j, lu) / (smueq * stheta));
        else
          phint += (D * ctheta * MU_EQ(j, lu) + C) * M_PI / 2.0;
      }
    }
...
/*
    ...
    O U T P U T     V A R I A B L E S:
       ...
       cmu,cwt     computational polar angles and corresponding quadrature
 weights dtaucpr expbea      transmission of direct beam flyr        separated
 fraction in delta-m method gl          phase function legendre coefficients
 multiplied by (2l+1) and single-scatter albedo layru       computational layer
 in which utau falls lyrcut      flag as to whether radiation will be zeroed
 below layer ncut ncut        computational layer where absorption optical depth
 first exceeds  abscut nn          ds->nstr/2 oprim       delta-m-scaled
 single-scatter albedo taucpr      delta-m-scaled optical depth utaupr
 delta-m-scaled version of  utau
...
void c_disort_set(disort_state *ds, double *ch, double *chtau, double *cmu,
                  double *cwt, int deltam, double *dtaucpr, double *expbea,
                  double *flyr, double *gl, int *layru, int *lyrcut, int *ncut,
                  int *nn, int *corint, double *oprim, double *tauc,
                  double *taucpr, double *utaupr) {
  ...
  /*
   * Apply delta-M scaling and move description of computational layers to local
   * variables
   */
  TAUCPR(0) = 0.;
  abstau = 0.;
  for (lc = 1; lc <= ds->nlyr; lc++) {
    ...
    if (!deltam) {
      ...
      for (k = 0; k <= ds->nstr - 1; k++) {
        GL(k, lc) =
            (double)(2 * k + 1) * OPRIM(lc) * (PMOM(k, lc) - f) / (1. - f);
      }
    }
  ...
  if ((ds->flag.ibcnd == GENERAL_BC && ds->bc.fbeam > 0.) ||
    ...
    if (ds->flag.spher == TRUE && ds->bc.umu0 < 0.) {
      EXPBEA(0) = exp(-c_chapman(1, 0., tauc, ds->nlyr, ds->zd, ds->dtauc,
                                 zenang, ds->radius));
    }
    if (ds->flag.spher == TRUE) {
      for (lc = 1; lc <= *ncut; lc++) {
        ...
        CHTAU(lc) = c_chapman(lc, 0., taucpr, ds->nlyr, ds->zd, dtaucpr, zenang,
                              ds->radius);
        chtau_tmp = c_chapman(lc, 0.5, taucpr, ds->nlyr, ds->zd, dtaucpr,
                              zenang, ds->radius);
  ...
  /*
   * If no thermal emission, cut off medium below absorption optical depth =
   * abscut ( note that delta-M transformation leaves absorption optical depth
   * invariant ).  Not worth the trouble for one-layer problems, though.
   */
  *lyrcut = FALSE;
  if (abstau >= abscut && !ds->flag.planck && ds->flag.ibcnd != SPECIAL_BC &&
      ds->nlyr > 1) {
    *lyrcut = TRUE;
  }
  ...
  /*
   * Set arrays defining location of user output levels within delta-M-scaled
   * computational mesh
   */
  for (lu = 1; lu <= ds->ntau; lu++) {
    for (lc = 1; lc < ds->nlyr; lc++) {
  ...
  *nn = ds->nstr / 2;
  ...
  if (ds->flag.ibcnd == GENERAL_BC && ds->bc.fbeam > 0.) {
    ...
    for (iq = 1; iq <= *nn; iq++) {
      if (fabs(ds->bc.umu0 - CMU(iq)) / fabs(ds->bc.umu0) < 1.e-4) {
        ...
        // c_errmsg("cdisort_set--beam angle=computational angle; change
        // ds.nstr",DS_ERROR);
      }
    }
  }
  ...
  if (ds->flag.usrang && ds->flag.ibcnd == SPECIAL_BC) {
    /*
     * Shift positive user angle cosines to upper locations and put negatives in
     * lower locations
     */
...
/*
    ...
    boundary conditions and the continuity-of-intensity-at-layer-interface
 equations.
   ...
   O U T P U T     V A R I A B L E S:
       ...
       cband    :  Left-hand side matrix of linear system eq. SC(5), scaled by
 eq. SC(12); in banded form required by LINPACK solution routines ncol     :
 Number of columns in cband
...
void c_set_matrix(disort_state *ds, double *bdr, double *cband, double *cmu,
                  double *cwt, double delm0, double *dtaucpr, double *gc,
                  double *kk, int lyrcut, int *ncol, int ncut, double *taucpr,
                  double *wk) {
  int mi = ds->nstr / 2, mi9m2 = 9 * mi - 2, nnlyri = ds->nstr * ds->nlyr,
      nn = ds->nstr / 2;
  ...
  *ncol = 0;
  ...
  /*
   * Use continuity conditions of eq. STWJ(17) to form coefficient matrix in
   * STWJ(20); employ scaling transformation STWJ(22)
   */
  for (lc = 1; lc <= ncut; lc++) {
    for (iq = 1; iq <= nn; iq++) {
...
double c_single_scat(double dither, int layru, int nlyr, double *phase,
                     double *omega, double *tau, double umu, double umu0,
                     double utau, double fbeam) {
  register int lyr;
  ...
  if (fabs(umu + umu0) <= dither) {
    ...
    ans = fbeam / (4. * M_PI * umu0) * exp0 *
          (ans + OMEGA(layru) * PHASE(layru) * (utau - TAU(layru - 1)));
    return ans;
  }
...
/*
   ...
   ** NOTE ** Eigenvalue problem is degenerate when single scattering
   ...
   I N T E R N A L   V A R I A B L E S:
       ...
       ab            :  Matrices AMB (alpha-beta), APB (alpha+beta) in reduced
 eigenvalue problem (see cdisort.h) array         :  Complete coefficient matrix
 of reduced eigenvalue problem: (alpha+beta)*(alpha-beta) gpplgm        :  (g+)
 + (g-) (cf. eqs. SS(10-11)) gpmigm        :  (g+) - (g-) (cf. eqs. SS(10-11))
...
/*
 * NOTE: Here the scratch array ARRAY(,) is half the size in each dimension
 * compared to other subroutines
 */
#undef ARRAY
#define ARRAY(iq, jq) array[iq - 1 + (jq - 1) * (ds->nstr / 2)]

void c_solve_eigen(disort_state *ds, int lc, disort_pair *ab, double *array,
                   double *cmu, double *cwt, double *gl, int mazim, int nn,
                   double *ylmc, double *cc, double *evecc, double *eval,
                   double *kk, double *gc, double *wk) {
  int ier;
  ...
  for (iq = 1; iq <= nn; iq++) {
    ...
    for (jq = 1; jq <= nn; jq++) {
      /*
       * Fill remainder of array using symmetry relations  C(-mui,muj) =
       * C(mui,-muj) and C(-mui,-muj) = C(mui,muj)
       */
  ...
  /*
   * Find eigenvectors (G+) + (G-) from SS(10) and store temporarily in APB
   * array
   */
  for (jq = 1; jq <= nn; jq++) {
    for (iq = 1; iq <= nn; iq++) {
  ...
  for (jq = 1; jq <= nn; jq++) {
    for (iq = 1; iq <= nn; iq++) {
      ...
      gpmigm = EVECC(iq, jq);
      /*
       * Recover eigenvectors G+,G- from their sum and difference; stack them to
       * get eigenvectors of full system SS(7) (JQ = eigenvector number)
       */
      EVECC(iq, jq) = .5 * (gpplgm + gpmigm);
      EVECC(iq + nn, jq) = .5 * (gpplgm - gpmigm);
      /*
       * Eigenvectors corresponding to negative eigenvalues (corresp. to
...
/*
    ...
    I N P U T      V A R I A B L E S:
       ...
       zzg      :  Beam source vectors in eq. KS(10)for a general source
 constant over a layer plk      :  Thermal source vectors z0,z1 by solving eq.
 SS(16), Y0,Y1 in STWL(26b,a); plk[].zero, plk[].one (see cdisort.h)
...
void c_solve0(disort_state *ds, double *b, double *bdr, double *bem,
              double bplanck, double *cband, double *cmu, double *cwt,
              double *expbea, int *ipvt, double *ll, int lyrcut, int mazim,
              int ncol, int ncut, int nn, double tplanck, double *taucpr,
              double *z, disort_pair *zbeamsp, double *zbeama, double *zz,
              double *zzg, disort_pair *plk) {
  ...
  if (mazim > 0 && (ds->bc.fbeam > 0. || ds->flag.general_source)) {
    ...
    if (lyrcut == TRUE || ds->flag.lamber == TRUE) {
      ...
      for (iq = 1; iq <= nn; iq++) {
        ...
        if (ds->flag.spher == TRUE) {
          ...
          sum = 0.;
          for (jq = 1; jq <= nn; jq++) {
            sum += CWT(jq) * CMU(jq) * BDR(iq, jq) * ZZ(nn + 1 - jq, ncut) *
                   EXPBEA(ncut);
          }
          B(ncol - nn + iq) = sum;
          if (ds->flag.general_source == TRUE) {
        ...
        if (ds->bc.fbeam > 0.) {
          if (ds->flag.spher == TRUE) {
            c_errmsg(
                "solve0--BDR not implemented for pseudo-spherical geometry",
                DS_WARNING);
          } else {
            B(ncol - nn + iq) +=
    ...
    if (ds->bc.fbeam == 0. && ds->flag.general_source == FALSE) {
      ...
      if (ds->flag.spher == TRUE) {
        for (iq = 1; iq <= nn; iq++)
          B(iq) = -ZBEAM0(nn + 1 - iq, 1) - ZPLK0(nn + 1 - iq, 1) +
                  ds->bc.fisot + tplanck;
      } else {
        for (iq = 1; iq <= nn; iq++)
          B(iq) = -ZZ(nn + 1 - iq, 1) - ZPLK0(nn + 1 - iq, 1) + ds->bc.fisot +
                  tplanck;
      }
      if (ds->flag.general_source == TRUE) {
        for (iq = 1; iq <= nn; iq++) B(iq) -= ZZG(nn + 1 - iq, 1);
        // aky	  B(iq) = B(iq) - ZZG(nn+1-iq,1);
      }
      if (lyrcut) {
        if (ds->flag.spher == TRUE) {
        ...
        if (ds->flag.general_source == TRUE) {
          for (iq = 1; iq <= nn; iq++) B(ncol - nn + iq) -= ZZG(iq + nn, ncut);
          // aky	    B(ncol-nn+iq) = B(ncol-nn+iq) - ZZG(iq+nn,ncut);
        }
      } else {
        if (ds->flag.spher == TRUE) {
          for (iq = 1; iq <= nn; iq++) {
            ...
            for (jq = 1; jq <= nn; jq++) {
              sum += CWT(jq) * CMU(jq) * BDR(iq, jq) *
                     (exp(-ZBEAMA(ncut) * TAUCPR(ncut)) *
                          (ZBEAM0(nn + 1 - jq, ncut) +
                           ZBEAM1(nn + 1 - jq, ncut) * TAUCPR(ncut)) +
                      ZZG(nn + 1 - jq, ncut) + ZPLK0(nn + 1 - jq, ncut) +
                      ZPLK1(nn + 1 - jq, ncut) * TAUCPR(ncut));
            }
            B(ncol - nn + iq) =
                2.0 * sum +
            ...
            for (jq = 1; jq <= nn; jq++) {
              sum += CWT(jq) * CMU(jq) * BDR(iq, jq) *
                     (ZZ(nn + 1 - jq, ncut) * EXPBEA(ncut) +
                      ZPLK0(nn + 1 - jq, ncut) + ZZG(nn + 1 - jq, ncut) +
                      ZPLK1(nn + 1 - jq, ncut) * TAUCPR(ncut));
            }
            B(ncol - nn + iq) =
                2. * sum +
        ...
        for (lc = 1; lc <= ncut - 1; lc++) {
          for (iq = 1; iq <= ds->nstr; iq++) {
            B(++it) =
                ...
                exp(-ZBEAMA(lc) * TAUCPR(lc)) *
                    (ZBEAM0(iq, lc) + ZBEAM1(iq, lc) * TAUCPR(lc)) +
                ZZG(iq, lc + 1) - ZZG(iq, lc) + ZPLK0(iq, lc + 1) -
                ZPLK0(iq, lc) +
                (ZPLK1(iq, lc + 1) - ZPLK1(iq, lc)) * TAUCPR(lc);
          }
        }
   ...
   */
  memset(cband, 0,
         (9 * (ds->nstr / 2) - 2) * (ds->nstr * ds->nlyr) * sizeof(double));
...
void c_surface_bidir(disort_state *ds, double delm0, double *cmu, int mazim,
                     int nn, double *bdr, double *emu, double *bem, double *rmu,
                     int callnum) {
  ...
  if (ds->flag.lamber && mazim == 0) {
    ...
    for (iq = 1; iq <= nn; iq++) {
      ...
      if (ds->bc.fbeam > 0.) {
        ...
        for (k = 1; k <= NMUG; k++) {
          sum += GWT(k) *
                 c_bidir_reflectivity(ds->wvnmlo, ds->wvnmhi, CMU(iq),
    ...
    if (mazim == 0) {
      /*
       * Integrate bidirectional reflectivity at reflection polar angle cosines
       * -CMU- and incident angle cosines -GMU- to get directional emissivity at
       * computational angle cosines -CMU-.
       */
      for (iq = 1; iq <= nn; iq++) {
        dref = 0.;
        for (jg = 1; jg <= NMUG; jg++) {
    ...
    for (iu = 1; iu <= ds->numu; iu++) {
      if (UMU(iu) > 0.) {
        if (ds->flag.lamber && mazim == 0) {
          ...
          if (mazim == 0) {
            /*
             * Integrate bidirectional reflectivity at reflection angle cosines
             * -UMU- and incident angle cosines -GMU- to get directional
             * emissivity at user angle cosines -UMU-.
             */
            dref = 0.;
            for (jg = 1; jg <= NMUG; jg++) {
              sum = 0.;
...
void c_interp_eigenvec(disort_state *ds, int lc, double *cwt, double *evecc,
                       double *gl, double *gu, int mazim, int nn, double *wk,
                       double *ylmc, double *ylmu) {
...
/*
    ...
    I N P U T      V A R I A B L E S:
       ...
       xr     :  Expansion of thermal source function, eq. STWL(24d); xr[].zero,
 xr[].one (see cdisort.h) ylm0   :  Normalized associated Legendre polynomial at
 the beam angle ylmc   :  Normalized associated Legendre polynomial at the
 quadrature angles ylmu   :  Normalized associated Legendre polynomial at the
 user angles zbs0   :  Solution vectors z-sub-zero of Eq. KS(10-11), used if
 pseudo-spherical zbs1   :  Solution vectors z-sub-one  of Eq. KS(10-11), used
 if pseudo-spherical zbsa   :  Alfa coefficient in Eq. KS(7), used if
 pseudo-spherical zee    :  Solution vectors Z-sub-zero, Z-sub-one of eq.
 SS(16), STWL(26a,b) zj     :  Solution vector Z-sub-zero after solving eq.
 SS(19), STWL(24b) zjg    :  Right-hand side vector  X-sub-zero in eq. KS(10),
 also the solution vector Z-sub-zero after solving that system for a general
 source constant over a layer

    O U T P U T     V A R I A B L E S:

       zbeam  :  Incident-beam source function at user angles
       zu     :  Components 0 and 1 of a linear-in-optical-depth-dependent
 source (approximating the Planck emission source) zgu    :  General source
 function at user angles
...
void c_interp_source(disort_state *ds, int lc, double *cwt, double delm0,
                     double *gl, int mazim, double *oprim, double *ylm0,
                     double *ylmc, double *ylmu, disort_pair *psi,
                     disort_pair *xr, disort_pair *zee, double *zj, double *zjg,
                     double *zbeam, disort_triplet *zbu, disort_pair *zbs,
                     double zbsa, double *zgu, disort_pair *zu) {
...
/*
    ...
    I N P U T      V A R I A B L E S:
       ...
       fbeam  :   incident beam radiation at top
       gl     :   Phase function Legendre coefficients multiplied by (2l+1) and
   single-scatter albedo lc:    :   layer index mazim  :   order of azimuthal
   component nstr   :   number of streams numu   :   number of user angles
       taucpr :   delta-m-scaled optical depth
       xba    :   alfa in eq. KS(7)
       ylmu   :   Normalized associated Legendre polynomial at the user angles
   -umu- ylm0   :   Normalized associated Legendre polynomial at the beam angle

    O U T P U T     V A R I A B L E S:

       zb0u   :   x-sub-zero in KS(7) at user angles -umu-
       zb1u   :   x-sub-one in KS(7) at user angles -umu-
       zju    :  Solution vector Z-sub-zero after solving eq. SS(19), STWL(24b),
   at user angles -umu-
...
void c_interp_coefficients_beam_source(disort_state *ds, double *chtau,
                                       double delm0, double fbeam, double *gl,
                                       int lc, int mazim, int nstr, int numu,
                                       double *taucpr, disort_triplet *zbu,
                                       double *xba, double *zju, double *ylm0,
                                       double *ylmu) {
  register int iu, k;
  ...
  for (iu = 1; iu <= numu; iu++) {
    ...
    ZB0U(iu, lc) =
        q0 * exp(XBA(lc) * TAUCPR(lc - 1)) - ZB1U(iu, lc) * TAUCPR(lc - 1);
  }
...
/*
    ...
    I N P U T      V A R I A B L E S:
       ...
       cmu    :   Computational polar angles
       ch     :   The Chapman-factor to correct for pseudo-spherical geometry in
 the direct beam. chtau  :   The optical depth in spherical geometry. delmo  :
 Kronecker delta, delta-sub-m0 fbeam  :   incident beam radiation at top gl :
 Phase function Legendre coefficients multiplied by (2l+1) and single-scatter
 albedo lc:    :   layer index mazim  :   order of azimuthal component nstr   :
 number of streams taucpr :   delta-m-scaled optical depth ylmc   :   Normalized
 associated Legendre polynomial at the quadrature angles -cmu- ylm0   :
 Normalized associated Legendre polynomial at the beam angle
...
void c_set_coefficients_beam_source(disort_state *ds, double *ch, double *chtau,
                                    double *cmu, double delm0, double fbeam,
                                    double *gl, int lc, int mazim, int nstr,
                                    double *taucpr, double *xba,
                                    disort_pair *xb, double *ylm0, double *ylmc,
                                    double *zj) {
  register int iq, k;
  double deltat, sum, q0a, q2a, q0, q2;
  static double big;
  ...
  for (iq = 1; iq <= nstr; iq++) {
    ...
    XB1(iq, lc) = (1.0 / deltat) * (q2 * exp(XBA(lc) * TAUCPR(lc)) -
                                    q0 * exp(XBA(lc) * TAUCPR(lc - 1)));
    XB0(iq, lc) =
        q0 * exp(XBA(lc) * TAUCPR(lc - 1)) - XB1(iq, lc) * TAUCPR(lc - 1);
  }
  return;
}
...
/*
   ...
   I N P U T    V A R I A B L E S:
       ...
       ylmc   :  Normalized associated Legendre polynomial at the quadrature
 angles
...
void c_upbeam(disort_state *ds, int lc, double *array, double *cc, double *cmu,
              double delm0, double *gl, int *ipvt, int mazim, int nn,
              double *wk, double *ylm0, double *ylmc, double *zj, double *zz) {
  ...
  /*
   * Find L-U (lower/upper triangular) decomposition of ARRAY and see if it is
   * nearly singular (NOTE:  ARRAY is altered)
   */
  rcond = 0.;
  ...
  /*
   * Solve linear system with coeff matrix ARRAY (assumed already L-U
   * decomposed) and R.H. side(s) ZJ; return solution(s) in ZJ
   */
...
void c_upbeam_pseudo_spherical(disort_state *ds, int lc, double *array,
                               double *cc, double *cmu, int *ipvt, int nn,
                               double *wk, disort_pair *xb, double *xba,
                               disort_pair *zbs, double *zbsa,
                               disort_pair *zbeamsp, double *zbeama) {
  register int iq, jq;
  double rcond, rmin;
...
/*
 ...
 angles
   ...
   O U T P U T    V A R I A B L E S:
       ...
       zjg    :  Right-hand side vector  X-sub-zero in eq. KS(10), also the
 solution vector Z-sub-zero after solving that system for a general source
 constant over a layer zzg    :  Permanent storage for zjg, but re-ordered
...
void c_upbeam_general_source(disort_state *ds, int lc, int maz, double *array,
                             double *cc, int *ipvt, int nn, double *wk,
                             double *zjg, double *zzg) {
  register int iq, jq;
  double rcond;
  ...
  if (1. + rcond == 1.) {
    c_errmsg("upbeam_general_source--sgeco says matrix near singular",
             DS_WARNING);
  }

  /*
   * Solve linear system with coeff matrix ARRAY (assumed already L-U
   * decomposed) and R.H. side(s) ZJG; return solution(s) in ZJG
   */
...
/*
    ...
    I N P U T     V A R I A B L E S:
       ...
       xr     :  Expansion coefficient b-sub-zero, b-sub-one of thermal source
 function, eq. STWL(24c)

    O U T P U T    V A R I A B L E S:

       zee    :  Solution vectors Z-sub-zero, Z-sub-one of eq. SS(16),
 STWL(26a,b) plk    :  Permanent storage for zee, but re-ordered
...
void c_upisot(disort_state *ds, int lc, double *array, double *cc, double *cmu,
              int *ipvt, int nn, double *oprim, double *wk, disort_pair *xr,
              disort_pair *zee, disort_pair *plk) {
  register int iq, jq;
  double rcond;
...
/*
   ...
   I N P U T    V A R I A B L E S:
       ...
       zu     :  Z-sub-zero, Z-sub-one in eq. SS(16) interpolated to user angles
 from an equation derived from SS(16), Y-sub-zero, Y-sub-one on STWL(26b,a);
 zu[].zero, zu[].one (see cdisort.h) zz     :  Beam source vectors in eq.
 SS(19), STWL(24b) zzg    :  Beam source vectors in eq. KS(10)for a general
 source constant over a layer plk    :  Thermal source vectors z0,z1 by solving
 eq. SS(16), Y-sub-zero,Y-sub-one in STWL(26) zbeam  :  Incident-beam source
 vectors
...
void c_user_intensities(disort_state *ds, double bplanck, double *cmu,
                        double *cwt, double delm0, double *dtaucpr, double *emu,
                        double *expbea, double *gc, double *gu, double *kk,
                        int *layru, double *ll, int lyrcut, int mazim, int ncut,
                        int nn, double *rmu, double *taucpr, double tplanck,
                        double *utaupr, double *wk, disort_triplet *zbu,
                        double *zbeam, disort_pair *zbeamsp, double *zbeama,
                        double *zgu, disort_pair *zu, double *zz, double *zzg,
                        disort_pair *plk, double *uum) {
  register int negumu, iq, iu, jq, lc, lu, lyrend, lyrstr, lyu;
  double alfa, bnddfu, bnddir, bndint, denom, dfuint, dtau, dtau1, dtau2,
      exp0 = 0, exp1 = 0, exp2 = 0, expn, f0n, f1n, fact, genint, palint,
      plkint, sgn;
  ...
  for (lu = 1; lu <= ds->ntau; lu++) {
    ...
    for (iu = 1; iu <= ds->numu; iu++) {
      ...
      /*
       * For downward intensity, integrate from top to LYU-1 in eq. S1(8); for
       * upward, integrate from bottom to LYU+1 in S1(9)
       */
      genint = 0.;
      palint = 0.;
      plkint = 0.;
      ...
      /*
       * Calculate contribution from user output level to next computational
       * level
       */
      dtau1 = UTAUPR(lu) - TAUCPR(lyu - 1);
      dtau2 = UTAUPR(lu) - TAUCPR(lyu);

      if ((fabs(dtau1) >= 1.e-6 || !negumu) &&
          (fabs(dtau2) >= 1.e-6 || negumu)) {
        if (negumu) {
        ...
        if (ds->bc.fbeam > 0.) {
          if (ds->flag.spher == TRUE) {
            if (negumu) {
              ...
              palint += ZB0U(iu, lyu) * denom *
                            (-exp(-alfa * UTAUPR(lu)) +
                             expn * exp(-alfa * TAUCPR(lyu - 1))) +
                        ZB1U(iu, lyu) * denom *
                            (-(UTAUPR(lu) - UMU(iu) * denom) *
                                 exp(-alfa * UTAUPR(lu)) +
                             (TAUCPR(lyu - 1) - UMU(iu) * denom) * expn *
                                 exp(-alfa * TAUCPR(lyu - 1)));
            } else {
              expn = exp2;
              alfa = ZBAU(iu, lyu);
        ...
        for (iq = 1; iq <= nn; iq++) {
          ...
          if (fabs(denom) < 0.0001) {
            ...
            expn =
                (exp(-KK(iq, lyu) * dtau2) - exp(KK(iq, lyu) * dtau) * exp1) /
                denom;
          } else {
        ...
        for (iq = nn + 1; iq <= ds->nstr; iq++) {
          ...
          if (fabs(denom) < 0.0001) {
            ...
            expn =
                (exp(-KK(iq, lyu) * dtau1) - exp(-KK(iq, lyu) * dtau) * exp2) /
                denom;
          }
          palint += GU(iu, iq, lyu) * expn;
        }
      ...
      if (negumu && mazim == 0) {
        ...
        for (iq = nn; iq >= 1; iq--) {
          ...
          if (ds->bc.fbeam > 0.) {
            if (ds->flag.spher == TRUE) {
              dfuint += exp(-ZBEAMA(ds->nlyr) * TAUCPR(ds->nlyr)) *
                        (ZBEAM0(iq, ds->nlyr) +
                         ZBEAM1(iq, ds->nlyr) * TAUCPR(ds->nlyr));
            } else {
          ...
          dfuint += delm0 * (ZPLK0(iq, ds->nlyr) +
                             ZPLK1(iq, ds->nlyr) * TAUCPR(ds->nlyr));
          bnddfu += (1. + delm0) * RMU(iu, nn + 1 - iq) * CMU(nn + 1 - iq) *
                    CWT(nn + 1 - iq) * dfuint;
        }
        bnddir = 0.;
        if (ds->bc.fbeam > 0. || ds->bc.umu0 > 0.) {
          bnddir =
              ds->bc.umu0 * ds->bc.fbeam / M_PI * RMU(iu, 0) * EXPBEA(ds->nlyr);
        }
        bndint = (bnddfu + bnddir + delm0 * EMU(iu) * bplanck + ds->bc.fluor) *
                 exp((UTAUPR(lu) - TAUCPR(ds->nlyr)) / UMU(iu));
      }
...
double c_xi_func(double umu1, double umu2, double tau) {
  ...
  if (x1 != 0.) {
    return ((tau * x1 - 1.) * exp(-tau / umu2) + exp1) /
           (x1 * x1 * umu1 * umu2);
  } else {
...
int c_check_inputs(disort_state *ds, int scat_yes, int deltam, int corint,
                   double *tauc, int callnum) {
  int inperr = FALSE;
  register int irmu, iu, j, k, lc, lu, nu;
  double flxalb, rmu, umumin;
  ...
  if (ds->nstr == 2) {
    c_errmsg(
        "check_inputs()--2 streams not recommended;\n\nUse specialized "
        "2-stream code c_twostr() instead",
        DS_WARNING);
  }
  if (ds->nlyr < 1) {
  ...
  if (ds->flag.ibcnd == GENERAL_BC) {
    if (ds->flag.usrtau) {
      ...
      for (lu = 1; lu <= ds->ntau; lu++) {
         ...
         */
        /* This might happen due to numerical rounding off problems.
         * ak20110224*/
  ...
  if (ds->flag.ibcnd == GENERAL_BC) {
    ...
    if (ds->flag.lamber) {
      ...
      /*
       * Make sure flux albedo at dense mesh of incident angles does not assume
       * unphysical values
       */
      for (irmu = 0; irmu <= 100; irmu++) {
        rmu = (double)irmu * 0.01;
        flxalb = c_dref(ds->wvnmlo, ds->wvnmhi, rmu, ds->flag.brdf_type,
                        &ds->brdf, callnum);
  ...
  if (ds->flag.planck && ds->flag.quiet == VERBOSE) {
    for (lc = 1; lc <= ds->nlyr; lc++) {
      if (fabs(TEMPER(lc) - TEMPER(lc - 1)) > 10.) {
        c_errmsg(
            "check_inputs--vertical temperature step may be too large for good "
            "accuracy",
            DS_WARNING);
      }
    }
  ...
  if (!corint &&
    ...
    c_errmsg(
        "check_inputs--intensity correction is off;\nintensities may be less "
        "accurate",
        DS_WARNING);
  }
...
double c_dref(double wvnmlo, double wvnmhi, double mu, int brdf_type,
              disort_brdf *brdf, int callnum) {
  static int pass1 = TRUE;
  register int jg, k;
  double ans, sum;
  static double gmu[NMUG], gwt[NMUG];
  ...
  for (jg = 1; jg <= NMUG; jg++) {
    ...
    for (k = 1; k <= NMUG / 2; k++) {
      sum += GWT(k) * GMU(k) *
             c_bidir_reflectivity(wvnmlo, wvnmhi, GMU(k), mu, M_PI * GMU(jg),
                                  brdf_type, brdf, callnum);
    }
    ans += GWT(jg) * sum;
  }
  if (ans < 0. || ans > 1.) {
...
/*
       ...
       REFERENCE: Dave, J.V. and B.H. Armstrong, Computations of High-Order
                    Associated Legendre Polynomials, J. Quant. Spectrosc.
 Radiat. Transfer 10, 557-562, 1970. (hereafter D/A)
  ...
  I N P U T     V A R I A B L E S:
       ...
       If m > 0, YLM(m-1,i) for i = 1 to nmu is assumed to exist from a prior
 call.
...
void c_legendre_poly(int nmu, int m, int maxmu, int twonm1, double *mu,
                     double *ylm) {
  register int i, l;
  ...
  if (m == 0) {
    ...
    for (l = 2; l <= twonm1; l++) {
      for (i = 1; i <= nmu; i++) {
        YLM(l, i) = ((double)(2 * l - 1) * MU(i) * YLM(l - 1, i) -
                     (double)(l - 1) * YLM(l - 2, i)) /
                    l;
      }
    }
  } else {
    for (i = 1; i <= nmu; i++) {
       ...
       */
      YLM(m, i) =
          -sqrt((1. - 1. / (2 * m)) * (1. - SQR(MU(i)))) * YLM(m - 1, i);
...
/*
  ...
  OUTPUT : ans    : Integrated Planck function ( Watts/sq m ) = Integral (WNUMLO
 to WNUMHI) of 2h c*c nu*nu*nu / ( exp(hc nu/kT) - 1) (where h=Plancks constant,
 c=speed of light, nu=wavenumber, T=temperature, and k = Boltzmann constant)

  Reference : Specifications of the Physical World: New Value of the Fundamental
 Constants, Dimensions/N.B.S., Jan. 1974

  Method :  For wnumlo close to wnumhi, a Simpson-rule quadrature is done to
 avoid ill-conditioning; otherwise

            (1)  For wnumlo or wnumhi small, integral(0 to WNUMLO/HI) is
 calculated by expanding the integrand in a power series and integrating term by
 term;

            (2)  Otherwise, integral (wnumlo/hi to infinity) is calculated by
 expanding the denominator of the integrand in powers of the exponential and
 integrating term by term.

  Accuracy :  At least 6 significant digits, assuming the physical constants are
 infinitely accurate
  ...
  ERRORS WHICH ARE NOT TRAPPED:
      ...
      * Simpson-rule special case is skipped when denominator of integrand will
 cause overflow. In that case the normal procedure is used, which may be
 inaccurate if the wavenumber limits (wnumlo, wnumhi) are close together.
...
double c_planck_func1(double wnumlo, double wnumhi, double t) {
  ...
  if (v[0] > DBL_EPSILON && v[1] < vmax && (wnumhi - wnumlo) / wnumhi < 1.e-2) {
    /*
     * Wavenumbers are very close.  Get integral by iterating Simpson rule to
     * convergence.
     */
    hh = v[1] - v[0];
    oldval = 0.;
  ...
  for (i = 1; i <= 2; i++) {
    if (v[i - 1] < VCUT) {
      ...
      p[i - 1] =
          ...
          (A1 +
           v[i - 1] *
               (A2 + v[i - 1] * (A3 + vsq * (A4 + vsq * (A5 + vsq * A6)))));
    } else {
      /*
...
void c_print_avg_intensities(disort_state *ds, disort_output *out) {
  ...
  fprintf(stdout,
          "\n\n *******  AZIMUTHALLY AVERAGED INTENSITIES (at user polar "
          "angles)  ********\n");
  lenfmt = 8;
...
void c_print_inputs(disort_state *ds, double *dtaucpr, int scat_yes, int deltam,
                    int corint, double *flyr, int lyrcut, double *oprim,
                    double *tauc, double *taucpr) {
  ...
  if (ds->flag.ibcnd == GENERAL_BC) {
    ...
    fprintf(stdout,
            "    Incident beam with intensity =%11.3e and polar angle cosine = "
            "%8.5f  and azimuth angle =%7.2f\n",
    ...
    if (ds->flag.planck) {
      fprintf(stdout,
              ...
              ds->wvnmlo, ds->wvnmhi);
      fprintf(stdout,
              "    Bottom temperature =%10.2f    Top temperature =%10.2f    "
              "Top emissivity =%8.4f\n",
  ...
  if (ds->flag.ibcnd == SPECIAL_BC) {
    fprintf(stdout,
            " Calculate albedo and transmissivity of medium vs. incident beam "
            "angle\n");
  ...
  fprintf(stdout,
          " Relative convergence criterion for azimuth series =%11.2e\n",
          ds->accur);
  ...
  if (ds->flag.planck) {
    ...
    fprintf(stdout,
            ...
            "Total    Single");
    fprintf(stdout,
            "\n       Optical   Optical   Scatter   Separated   Optical   "
            "Optical   Scatter    Asymm");
    fprintf(stdout,
            "\n         Depth     Depth    Albedo    Fraction     Depth     "
            "Depth    Albedo   Factor   Temperature\n");
  } else {
    fprintf(stdout,
            "\n                                     <------------- Delta-M "
            "--------------->");
  ...
  for (lc = 1; lc <= ds->nlyr; lc++) {
    if (ds->flag.planck) {
      fprintf(stdout,
              ...
              DTAUC(lc), TAUC(lc), SSALB(lc), FLYR(lc), DTAUCPR(lc), TAUCPR(lc),
              OPRIM(lc), PMOM(1, lc), TEMPER(lc - 1));
    } else {
      fprintf(stdout, "%4d%10.4f%10.4f%10.5f%12.5f%10.4f%10.4f%10.5f%9.4f\n",
              lc, DTAUC(lc), TAUC(lc), SSALB(lc), FLYR(lc), DTAUCPR(lc),
...
/*
   ...
   METHOD:     Compute the abscissae as roots of the Legendre polynomial P-sub-M
 using a cubically convergent refinement of Newton's method.  Compute the
 weights from eq. 2.7.3.8 of Davis/Rabinowitz.  Note that Newton's method can
 very easily diverge; only a very good initial guess can guarantee convergence.
               The initial guess used here has never led to divergence even for
 M up to 1000.

   ACCURACY:   Relative error no better than TOL or computer precision
 (DBL_EPSILON), whichever is larger
   ...
   INTERNAL VARIABLES:
    ...
    x,xi      : Successive iterates in cubically-convergent version of Newtons
 Method (seeking roots of Legendre poly.)
...
void c_gaussian_quadrature(int m, double *gmu, double *gwt) {
  ...
  for (k = 1; k <= lim; k++) {
    /*
     * Initial guess for k-th root of Legendre polynomial, from Davis/Rabinowitz
     * (2.7.3.3a)
     */
...
void c_self_test(int compare, int *prntu0, disort_state *ds,
                 disort_output *out) {
  ...
  if (compare == FALSE) {
    ...
    if (fabs(error) > acc) {
      ok = FALSE;
      fprintf(stderr,
              "Output variable uu differed by %g percent from correct value.\n",
              100. * error);
    }
    ...
    if (fabs(error) > acc) {
      ...
      fprintf(
          stderr,
          "Output variable rfldir differed by %g percent from correct value.\n",
          100. * error);
    }
    ...
    if (fabs(error) > acc) {
      ...
      fprintf(
          stderr,
          "Output variable rfldn differed by %g percent from correct value.\n",
          100. * error);
    }
    ...
    if (fabs(error) > acc) {
      ...
      fprintf(
          stderr,
          "Output variable flup differed by %g percent from correct value.\n",
          100. * error);
    }
...
/*
   DISORT special case to get only albedo and transmissivity of entire medium as
 a function of incident beam angle (many simplifications because boundary
 condition is just isotropic illumination, there are no thermal sources, and
   particular solutions do not need to be computed).  See Ref. S2 and references
 therein for details. The basic idea is as follows.  The reciprocity principle
 leads to the following relationships for a plane-parallel, vertically
 inhomogeneous medium lacking thermal (or other internal) sources:
   ...
   O U T P U T    V A R I A B L E S:
    ...
    I N T E R N A L   V A R I A B L E S:
       ...
       rcond       estimate of the reciprocal condition of matrix CBAND; for
 system  CBAND*X = B, relative perturbations in CBAND and B of size epsilon may
 cause relative perturbations in X of size epsilon/RCOND.  If RCOND is so small
 that 1.0 + RCOND .eq. 1.0 is true, then CBAND may be singular to working
 precision. cband       Left-hand side matrix of linear system eq. SC(5), scaled
 by eq. SC(12); in banded form required by LINPACK solution routines ncol number
 of columns in CBAND matrix ipvt        INTEGER vector of pivot indices (most
 others documented in DISORT)

   Called by- c_disort
   Calls- c_legendre_poly, c_sgbco, c_solve_eigen, c_interp_eigenvec,
 c_set_matrix, c_solve1, c_albtrans_intensity, c_albtrans_spherical,
 c_print_albtrans
 --------------------------------------------------------------------------------------*/

void c_albtrans(disort_state *ds, disort_output *out, disort_pair *ab,
                double *array, double *b, double *bdr, double *cband,
                double *cc, double *cmu, double *cwt, double *dtaucpr,
                double *eval, double *evecc, double *gl, double *gc, double *gu,
                int *ipvt, double *kk, double *ll, int nn, double *taucpr,
                double *ylmc, double *ylmu, double *z, double *wk) {
  int lyrcut, ncol;
  ...
  delm0 = 1.;
  /*
   * Set DISORT variables that are ignored in this special case but are needed
   * below in argument lists of subroutines shared with general case
   */
  ncut = ds->nlyr;
  lyrcut = FALSE;
  ds->bc.fisot = 1.;
  ...
  /*
   * Evaluate Legendre polynomials with negative arguments from those with
  ...
  /*
   * Zero out bottom reflectivity (ALBEDO is used only in analytic formulae
   * involving ALBEDO = 0 solutions; eqs 16-17 of Ref S2)
   */
  memset(bdr, 0, (ds->nstr / 2) * ((ds->nstr / 2) + 1) * sizeof(double));
   ...
   * LINPACK routines)
   */
  c_set_matrix(ds, bdr, cband, cmu, cwt, delm0, dtaucpr, gc, kk, lyrcut, &ncol,
               ncut, taucpr, wk);
  ...
  /*
   * Compute azimuthally-averaged intensity at user angles; gives albedo if
   * multi-layer (eq. 9 of Ref S2); gives both albedo and transmissivity if
   * single layer (eqs. 3-4 of Ref S2)
   */
  ...
  if (ds->nlyr == 1) {
    for (iu = 1; iu <= ds->numu / 2; iu++) {
      /*
       * Get beam-incidence transmissivities from reciprocity principle (1
       * layer); flip them end over end to correspond to positive UMU instead of
       * negative
       */
      TRNMED(iu) = U0U(ds->numu / 2 + 1 - iu, 2) +
                   exp(-TAUCPR(ds->nlyr) / UMU(iu + ds->numu / 2));
    }
  } else {
    /*
  ...
  if (ds->bc.albedo > 0.) {
     ...
     */
    if (ds->nlyr == 1) {
      c_albtrans_spherical(ds, cmu, cwt, gc, kk, ll, nn, taucpr, &sphalb,
                           &sphtrn);
    } else {
      c_albtrans_spherical(ds, cmu, cwt, gc, kk, ll, nn, taucpr, &sphtrn,
                           &sphalb);
    }
    /*
    ...
    for (iu = 1; iu <= ds->numu; iu++) {
      ALBMED(iu) +=
          ds->bc.albedo / (1. - ds->bc.albedo * sphalb) * sphtrn * TRNMED(iu);
      TRNMED(iu) +=
          ds->bc.albedo / (1. - ds->bc.albedo * sphalb) * sphalb * TRNMED(iu);
    }
  }
  /*
  ...
  /* CE: I want to output the the spherical albedo and transmittance, and use
   * the */
  /* variables ALBMED and TRNMED for this. They are not used so far otherwise in
   * uvspec */
  /* If somebody needs these variables I will include new variables for sphtrn
   * and sphalb*/
  ALBMED(1) = sphalb;
  TRNMED(1) = sphtrn;
...
/*
   ...
   ** NOTE **  User input values of UMU (assumed positive) are temporarily in
   ...
   I N P U T    V A R I A B L E S:
       ...
       ds     :  Disort state variables
       gu     :  Eigenvectors interpolated to user polar angles (i.e., g in eq.
 SC(1), STWL(31ab)) kk     :  Eigenvalues of coeff. matrix in eq. SS(7),
 STWL(23b) ll     :  Constants of integration in eq. SC(1), obtained by solving
 scaled version of eq. SC(5); exponential term of eq. SC(12) not included nn :
 Order of double-Gauss quadrature (NSTR/2) taucpr :  Cumulative optical depth
 (delta-M-scaled)

   O U T P U T    V A R I A B L E:

       out->u0u : Diffuse azimuthally-averaged intensity at top and bottom of
 medium (directly transmitted component, corresponding to bndint in
 user_intensities, is omitted).
...
void c_albtrans_intensity(disort_state *ds, disort_output *out, double *gu,
                          double *kk, double *ll, int nn, double *taucpr,
                          double *wk) {
...
/*
     ...
     I N P U T      V A R I A B L E S:
       ...
       ihom     :  Direction-of-illumination flag (TOP_ILLUM, top; BOT_ILLUM,
 bottom) ipvt     : ncol     :  Number of columns in CBAND ncut     : nn       :
 Order of double-Gauss quadrature (NSTR/2)
...
void c_solve1(disort_state *ds, double *cband, int ihom, int *ipvt, int ncol,
              int ncut, int nn, double *b, double *ll) {
  ...
  if (ihom == TOP_ILLUM) {
    /*
     * Because there are no beam or emission sources, remainder of B array is
     * zero
     */
    for (i = 1; i <= nn; i++) {
      B(i) = ds->bc.fisot;
...
/*
    ...
    I N P U T    V A R I A B L E S:
       ...
       gc      :  Eigenvectors at polar quadrature angles, SC(1)
       ll      :  Constants of integration in eq. SC(1), obtained by solving
                  scaled version of eq. SC(5); exponential term of eq. SC(12)
 not incl. nn      :  Order of double-Gauss quadrature (NSTR/2)
...
void c_albtrans_spherical(disort_state *ds, double *cmu, double *cwt,
                          double *gc, double *kk, double *ll, int nn,
                          double *taucpr, double *sflup, double *sfldn) {
  register int iq, jq;
  double zint;
  ...
  *sfldn = 0.;
  for (iq = 1; iq <= nn; iq++) {
    ...
    for (jq = nn + 1; jq <= ds->nstr; jq++) {
      zint +=
          GC(iq, jq, ds->nlyr) * LL(jq, ds->nlyr) *
          exp(-KK(jq, ds->nlyr) * (TAUCPR(ds->nlyr) - TAUCPR(ds->nlyr - 1)));
    }
...
void c_errmsg(char const *messag, int type) {
  ...
  if (++num_warnings <= MAX_WARNINGS) {
    ...
    fprintf(stderr,
            "\n\n >>>>>>  TOO MANY WARNING MESSAGES --  ','They will no longer "
            "be printed  <<<<<<<\n\n");
    warning_limit = TRUE;
  }
...
int c_write_too_small_dim(int quiet, char const *dimnam, int minval) {
  if (quiet != QUIET) {
    fprintf(stderr,
            " ****  Symbolic dimension %s should be increased to at least %d  "
            "****\n",
            dimnam, minval);
  }
...
void c_sgbco(double *abd, int lda, int n, int ml, int mu, int *ipvt,
...
void c_sgbfa(double *abd, int lda, int n, int ml, int mu, int *ipvt,
             int *info) {
...
void c_sgbsl(double *abd, int lda, int n, int ml, int mu, int *ipvt, double *b,
             int job) {
...
double c_sdot(int n, double *sx, double *sy) {
   ...
   */
  for (i = m + 1; i <= n; i += 4) {
    ans += SX(i) * SY(i) + SX(i + 1) * SY(i + 1) + SX(i + 2) * SY(i + 2) +
           SX(i + 3) * SY(i + 3);
  }
...
+---------------------------------------------------------------------+
    ...
    ********  User level organization  ********
    ...
    UTAU(lu)       lu = 1 to ds.ntau, user optical depths, in increasing order.
                   UTAU(ntau) must be no greater than the total optical depth of
the medium.
    ...
    **********  Control flags  **************
    ...
    ds.flag.planck  = TRUE, include thermal emission
                     ...
                     ( If ds.flag.planck = FALSE, it is not necessary to set any
of the variables having to do with thermal emission )
...
+---------------------------------------------------------------------+
    ...
    IERROR(i)     :   Error flag array, if IERROR(i) is zero everything
                      ...
                      case, twostr return immediately and reports the error in
IERROR.
                     ...
                     NOTE: i = 2, 9, 11, 20, and 21 are eliminated in the C
version by the change from static to dynamic memory allocation
...
+---------------------------------------------------------------------+
      Routines called (in order): c_twostr_check_inputs, c_twostr_set,
c_twostr_print_inputs, c_twostr_solns, c_set_matrix, c_twostr_solve_bc,
c_twostr_fluxes
+---------------------------------------------------------------------+
  ...
  Index conventions (for all loops and all variable descriptions):
     ...
     lu     :  For user levels
     lc     :  For computational layers (each having a different single-scatter
albedo and/or phase function) lev    :  For computational levels ls     :  Runs
from 0 to 2*ds->nlyr+1, ls = 1,2,3 refers to top, center and bottom of layer 1,
...
+---------------------------------------------------------------------+
   ...
   B()...........Right-hand side vector of eqs. KST(38-41), set in
twostr_solve_bc() bplanck.......Intensity emitted from bottom boundary
   CBAND().......Matrix of left-hand side of the linear system eqs. KST(38-41);
in tridiagonal form CH(lc)........The Chapman-factor to correct for
pseudo-spherical geometry in the direct beam. CHTAU(lc).....The optical depth in
spherical geometry. cmu...........Computational polar angle, single or double
Gaussian quadrature rule used, see twostr_set() EXPBEA(lc)....Transmission of
direct beam in delta-m optical depth coordinates FLDIR(lu).....Direct beam flux
(delta-m scaled); fl[].zero (see cdisort.h) FLDN(lu)......Diffuse down flux
(delta-m scaled); fl[].one (see cdisort.h) FLYR(lc)......Truncated fraction in
delta-m method KK(lc)........Eigenvalues in eq. KST(20)
   LAYRU(lu).....Computational layer in which user output level UTAU(lu) is
located LL(iq,lc).....Constants of integration C-tilde in eqs. KST(42-43)
obtained by solving eqs. KST(38-41) lyrcut........True, radiation is assumed
zero below layer -ncut- because of almost complete absorption
   ncut..........Computational layer number in which absorption optical depth
first exceeds abscut OPRIM(lc).....Single scattering albedo after delta-m
scaling pass1.........TRUE on first entry, FALSE thereafter
   PKAG(0:lc)....Integrated Planck function for internal emission at layer
boundaries PKAGC(lc).....Integrated Planck function for internal emission at
layer center RR(lc)........Eigenvectors at polar quadrature angles.
   TAUC(0:lc)....Cumulative optical depth (un-delta-m-scaled)
   TAUCPR(0:lc)..Cumulative optical depth (delta-m-scaled if deltam = TRUE,
otherwise equal to TAUC) tplanck.......Intensity emitted from top boundary
   U0C(iq,lu)....Azimuthally-averaged intensity
   UTAUPR(lu)....Optical depths of user output levels in delta-m coordinates;
equal to UTAU(lu) if no delta-m

   The following are members of the structure twostr_xyz:
   XB_0D(lc).....x-sub-zero-sup-minus in expansion of pseudo-spherical beam
source, eq. KST(22) XB_0U(lc).....x-sub-zero-sup-plus  in expansion of
pseudo-spherical beam source, eq. KST(22) XB_1D(lc).....x-sub-one-sup-minus  in
expansion of pseudo-spherical beam source, eq. KST(22)
   XB_1U(lc).....x-sub-one-sup-plus   in expansion of pseudo-spherical beam
source, eq. KST(22) XP_0(lc)......x-sub-zero in expansion of thermal source
function; see eq. KST(22) (has no (mu) dependence) XP_1(lc)......x-sub-one  in
expansion of thermal source function; see eq. KST(22) (has no (mu) dependence)
   YB_0D(lc).....y-sub-zero-sup-minus in eq. KST(23), solution for
pseudo-spherical beam source YB_0U(lc).....y-sub-zero-sup-plus  in eq. KST(23),
solution for pseudo-spherical beam source YB_1D(lc).....y-sub-one-sup-minus  in
eq. KST(23), solution for pseudo-spherical beam source
   YB_1U(lc).....y-sub-one-sup-plus   in eq. KST(23), solution for
pseudo-spherical beam source YP_0D(lc).....y-sub-zero-sup-minus in eq. KST(23),
solution for thermal source YP_0U(lc).....y-sub-zero-sup-plus  in eq. KST(23),
solution for thermal source YP_1D(lc).....y-sub-one-sup-minus  in eq. KST(23),
solution for thermal source YP_1U(lc).....y-sub-one-sup-plus   in eq. KST(23),
solution for thermal source ZB_A(lc)......Alpha coefficient in eq. KST(22) for
pseudo-spherical beam source ZP_A(lc)......Alpha coefficient in eq. KST(22) for
thermal source
*/

void c_twostr(disort_state *ds, disort_output *out, int deltam, double *gg,
              int *ierror, double radius) {
  register int lc, ierr;
  int lyrcut, iret, ncut, nn;
  int ipvt[ds->nstr * ds->nlyr], layru[ds->ntau];
  double cmu, bplanck, tplanck;
  double *b, *cband, *ch, *chtau, *dtaucpr, *expbea, *flyr, *ggprim, *kk, *ll,
      *oprim, *pkag, *pkagc, *rr, *tauc, *taucpr, *u0c, *utaupr;
  disort_pair *fl;
  twostr_xyz *ts;
  twostr_diag *diag;
  const double dither = 100. * DBL_EPSILON;
  ...
  utaupr = c_dbl_vector(0, ds->ntau - 1, "utaupr");
  /*
   * Using C structures to facilitate cache-aware memory allocation, which tends
   * to reduce cache misses and speed up computer execution.
   */
  fl = (disort_pair *)calloc(ds->ntau, sizeof(disort_pair));
  if (!fl) c_errmsg("twostr alloc error for fl", DS_ERROR);
  ts = (twostr_xyz *)calloc(ds->nlyr, sizeof(twostr_xyz));
  if (!ts) c_errmsg("twostr alloc error for ts", DS_ERROR);
  diag = (twostr_diag *)calloc(2 * ds->nlyr, sizeof(twostr_diag));
  if (!diag) c_errmsg("twostr alloc error for diag", DS_ERROR);
  ...
  if (ds->flag.prnt[0]) {
    fprintf(stdout,
            ...
            " *****************************************************************"
            "*******************************************************\n"
            "                         Two stream method radiative transfer "
  ...
  /*
   * Calculate cumulative optical depth and dither single-scatter albedo to
   * improve numerical behavior of eigenvalue/vector computation
   */
   ...
   * Perform various setup operations
   */
  c_twostr_set(ds, &bplanck, ch, chtau, &cmu, deltam, dtaucpr, expbea, flyr, gg,
               ggprim, layru, &lyrcut, &ncut, &nn, oprim, pkag, pkagc, radius,
               tauc, taucpr, &tplanck, utaupr);
   ...
   */
  c_twostr_solns(ds, ch, chtau, cmu, ncut, oprim, pkag, pkagc, taucpr, ggprim,
                 kk, rr, ts);
   ...
   * boundary conditions)
   */
  c_twostr_solve_bc(ds, ts, bplanck, cband, cmu, expbea, lyrcut, nn, ncut,
                    tplanck, taucpr, kk, rr, ipvt, b, ll, diag);

  /*
   * Compute upward and downward fluxes, mean intensities and flux divergences.
   */
  c_twostr_fluxes(ds, ts, ch, cmu, kk, layru, ll, lyrcut, ncut, oprim, rr,
                  taucpr, utaupr, out, u0c, fl);

  /*
   * Free allocated memory
   */
free_local_memory_and_return:
  free(b), free(cband), free(ch), free(chtau), free(dtaucpr), free(expbea),
      free(flyr), free(fl);
  free(ggprim), free(kk), free(ll), free(oprim), free(pkag), free(pkagc),
      free(rr), free(tauc), free(taucpr);
...
/*
 ...
 I n p u t       v a r i a b l e s:
      ...
      r         : Radial parameter, see Velinow & Kostov (2001). NOTE: Use the
 same dimension as zd, for instance both in km.

 O u t p u t      v a r i a b l e s:

      ch        : Chapman-factor. In a pseudo-spherical atmosphere, replace
 exp(-tau/umu0) by exp(-ch(lc)) in the beam source in
 ...
 NOTE: Assumes a spherical planet. One might consider generalizing following
       Velinow YPI, Kostov VI, 2001, Generalization on Chapman Function for the
 Atmosphere of an Oblate Rotating Planet, Comptes Rendus de l'Academie Bulgare
 des Sciences 54, 29-34.
*/

double c_chapman(int lc, double taup, double *tauc, int nlyr, double *zd,
                 double *dtau_c, double zenang, double r) {
  register int id, j;
  ...
  if (id > lc) {
    ...
    dsj = sqrt(xp * xp - xpsinz * xpsinz) -
          sqrt(SQR(ZD(lc) + r) - xpsinz * xpsinz);
...
double c_chapman_simpler(int lc, double taup, int nlyr, double *zd,
                         double *dtau_c, double zenang, double r) {
  register int id, j;
  double zenrad, xp, xpsinz, sum, fact, fact2, rj, rjp1, dhj, dsj;
...
void c_twostr_check_inputs(disort_state *ds, double *gg, int *ierror,
                           double *tauc) {
  int inperr, lc, lu;
  double umumin;
  ...
  if (ds->flag.usrtau) {
    ...
    for (lu = 1; lu <= ds->ntau; lu++) {
      if (fabs(UTAU(lu) - TAUC(ds->nlyr)) <=
          1.e-6 *
              TAUC(
                  ds->nlyr)) { /* relative check copied from c_check_inputs() */
        UTAU(lu) = TAUC(ds->nlyr);
      }
      if (UTAU(lu) < 0. || UTAU(lu) > TAUC(ds->nlyr)) {
  ...
  for (lc = 1; lc <= ds->nlyr; lc++) {
    if (ds->flag.planck && fabs(TEMPER(lc) - TEMPER(lc - 1)) > 50. &&
        ds->flag.quiet == VERBOSE) {
      c_errmsg(
          "twostr_check_inputs--vertical temperature step may be too large for "
          "good accuracy",
          DS_WARNING);
    }
  }
...
/*
 ...
 I n p u t     v a r i a b l e s:
   ...
   ds         :  'Disort' state variables
   ts         :  twostr_xyz structure variables (xp_0, yb_0d, zb_a...; see
 cdisort.h) ch         :  Chapman factor cmu        :  Abscissa for gauss
 quadrature over angle cosine kk         :  Eigenvalues layru      :  Layer
 numbers of user levels -utau- ll         :  Constants of integration in eqs.
 KST(42-43), obtaine by solving eqs. KST(38-41) lyrcut     :  Logical flag for
 truncation of comput. layer ncut       :  Number of computational layer where
 absorption optical depth exceeds -abscut- oprim      :  Delta-m scaled single
 scattering albedo rr         :  Eigenvectors at polar quadrature angles
   flag.spher :  TRUE turns on pseudo-spherical effects
   taucpr     :  Cumulative optical depth (delta-m-scaled)
   utaupr     :  Optical depths of user output levels in delta-m coordinates;
 equal to  -utau- if no delta-m
...
void c_twostr_fluxes(disort_state *ds, twostr_xyz *ts, double *ch, double cmu,
                     double *kk, int *layru, double *ll, int lyrcut, int ncut,
                     double *oprim, double *rr, double *taucpr, double *utaupr,
                     disort_output *out, double *u0c, disort_pair *fl) {
  register int lu, lyu;
  ...
  for (lu = 1; lu <= ds->ntau; lu++) {
    ...
    if (lyrcut && lyu > ncut) {
      ...
      /*
       * Calculate fluxes and mean intensities; downward and upward fluxes from
       * eq. KST(9)
       */
      fact1 = 2. * M_PI * cmu;
...
/*
    ...
    I n p u t     v a r i a b l e s:
      ...
      ncut       : Number of computational layer where absorption optical depth
  exceeds -abscut- oprim      : Delta-m scaled single scattering albedo pkag,c
  : Planck function in each layer flag.spher : spher = true => spherical
  geometry invoked taucpr     : Cumulative optical depth (delta-m-scaled) ggprim
  :
...
void c_twostr_solns(disort_state *ds, double *ch, double *chtau, double cmu,
                    int ncut, double *oprim, double *pkag, double *pkagc,
                    double *taucpr, double *ggprim, double *kk, double *rr,
                    twostr_xyz *ts) {
  register int lc;
  static int initialized = FALSE;
  static double big, large, small, little;
  double q_1, q_2, qq, q0a, q0, q1a, q2a, q1, q2, deltat, denomb, z0p, z0m, arg,
      sgn, fact3, denomp, beta, fact1, fact2;

  if (!initialized) {
    /*
     * The calculation of the particular solutions require some care;
     small,little, big, and large have been set so that no problems should occur
     in double precision.
     */
  ...
  for (lc = 1; lc <= ncut; lc++) {
    ...
    if (ds->bc.fbeam > 0.) {
       ...
       */
      q_1 = ds->bc.fbeam / (4. * M_PI) * OPRIM(lc) *
            (1. - 3. * GGPRIM(lc) * cmu * ds->bc.umu0);
      q_2 = ds->bc.fbeam / (4. * M_PI) * OPRIM(lc) *
            (1. + 3. * GGPRIM(lc) * cmu * ds->bc.umu0);
      ...
      deltat = TAUCPR(lc) - TAUCPR(lc - 1);
      ZB_A(lc) = 1. / CH(lc);
      if (fabs(ZB_A(lc) * TAUCPR(lc - 1)) > large ||
          fabs(ZB_A(lc) * TAUCPR(lc)) > large) {
        ZB_A(lc) = 0.;
      }
      ...
      if (deltat < 1.e-07) {
        ...
        XB_1D(lc) = 1. / deltat *
                    (q2 * exp(ZB_A(lc) * TAUCPR(lc)) -
                     q0 * exp(ZB_A(lc) * TAUCPR(lc - 1)));
      }
      XB_0D(lc) =
          q0 * exp(ZB_A(lc) * TAUCPR(lc - 1)) - XB_1D(lc) * TAUCPR(lc - 1);
      q0 = q0a * q_2;
      q2 = q2a * q_2;
      ...
      XB_0U(lc) =
          q0 * exp(ZB_A(lc) * TAUCPR(lc - 1)) - XB_1U(lc) * TAUCPR(lc - 1);
      ...
      denomb = fact1 * fact2 - SQR(ZB_A(lc) * cmu);
      YB_1D(lc) =
          (OPRIM(lc) * beta * XB_1D(lc) +
           (1. - OPRIM(lc) * (1. - beta) + ZB_A(lc) * cmu) * XB_1U(lc)) /
          denomb;
      YB_1U(lc) =
          (OPRIM(lc) * beta * XB_1U(lc) +
           (1. - OPRIM(lc) * (1. - beta) - ZB_A(lc) * cmu) * XB_1D(lc)) /
          denomb;
      z0p = XB_0U(lc) - cmu * YB_1D(lc);
      z0m = XB_0D(lc) + cmu * YB_1U(lc);
      YB_0D(lc) = (OPRIM(lc) * beta * z0m +
                   (1. - OPRIM(lc) * (1. - beta) + ZB_A(lc) * cmu) * z0p) /
                  denomb;
      YB_0U(lc) = (OPRIM(lc) * beta * z0p +
                   (1. - OPRIM(lc) * (1. - beta) - ZB_A(lc) * cmu) * z0m) /
                  denomb;
    }
    ...
    if (ds->flag.planck) {
      ...
      if ((q2 < q0 * 1.e-02 || q2 <= little) && q1 > little && q0 > little) {
        ...
        XP_1(lc) = 0.;
      } else if ((q2 <= q1 * 1.e-02 || q2 <= little) &&
                 (q1 <= q0 * 1.e-02 || q1 <= little) && q0 > little) {
        /*
        ...
        XP_0(lc) = 0.;
        XP_1(lc) = 0.;
      } else if ((fabs((q2 - q0) / q2) < 1.e-04 &&
                  fabs((q2 - q1) / q2) < 1.e-04) ||
                 deltat < 1.e-04) {
        /*
         * Case 4: Sources same at center, bottom and top of layer or layer
         * optically very thin
         */
        ZP_A(lc) = 0.;
        XP_0(lc) = q0;
        ...
        if (deltat < 1.e-07) {
          ...
          XP_1(lc) = 1. / deltat *
                     (q2 * exp(ZP_A(lc) * TAUCPR(lc)) -
                      q0 * exp(ZP_A(lc) * TAUCPR(lc - 1)));
        }
        XP_0(lc) =
            q0 * exp(ZP_A(lc) * TAUCPR(lc - 1)) - XP_1(lc) * TAUCPR(lc - 1);
      }
      ...
      YP_1D(lc) = (OPRIM(lc) * beta * XP_1(lc) +
                   (1. - OPRIM(lc) * (1. - beta) + ZP_A(lc) * cmu) * XP_1(lc)) /
                  denomp;
      ...
      YP_0D(lc) = (OPRIM(lc) * beta * z0m +
                   (1. - OPRIM(lc) * (1. - beta) + ZP_A(lc) * cmu) * z0p) /
                  denomp;
      YP_0U(lc) = (OPRIM(lc) * beta * z0p +
                   (1. - OPRIM(lc) * (1. - beta) - ZP_A(lc) * cmu) * z0m) /
                  denomp;
    }
  }
 ...
 */
void c_twostr_print_inputs(disort_state *ds, int deltam, double *flyr,
                           double *gg, int lyrcut, double *oprim, double *tauc,
                           double *taucpr) {
  register int lu, lc;
  ...
  if (ds->flag.planck) {
    fprintf(stdout,
            "    Thermal emission in wavenumber interval :%14.4f%14.4f\n"
            "    bottom temperature =%10.2f     top temperature =%10.2f    top "
            "emissivity =%8.4f\n",
  ...
  if (ds->flag.planck) {
    fprintf(stdout,
            ...
            "\n                   total    single                           "
            "total    single"
            "\n       optical   optical   scatter   truncated   optical   "
            "optical   scatter    asymm"
            "\n         depth     depth    albedo    fraction     depth     "
            "depth    albedo   factor   temperature\n");
  } else {
  ...
  for (lc = 1; lc <= ds->nlyr; lc++) {
    if (ds->flag.planck) {
      fprintf(stdout,
              "%4d%10.4f%10.4f%10.5f%12.5f%10.4f%10.4f%10.5f%9.4f%14.3f\n", lc,
              DTAUC(lc), TAUC(lc), SSALB(lc), FLYR(lc),
              TAUCPR(lc) - TAUCPR(lc - 1), TAUCPR(lc), OPRIM(lc), GG(lc),
              TEMPER(lc - 1));
    } else {
      fprintf(stdout, "%4d%10.4f%10.4f%10.5f%12.5f%10.4f%10.4f%10.5f%9.4f\n",
...
/*
 ...
 Output:  ntau,utau  If ds->flag.usrtau = FALSE
          ...
          lyrcut     Flag as to whether radiation will be zeroed below layer
 ncut ncut       Computational layer where absorption optical depth first
 exceeds abscut nn         nstr/2 = 1 nstr       No.of streams (=2) oprim
 Delta-m-scaled single-scatter albedo pkag,c     Planck function in each layer
          taucpr     Delta-m-scaled optical depth
          tplanck    Intensity emitted from top boundary
          utaupr     Delta-m-scaled version of utau

 Internal Variables
          abscut     Absorption optical depth, medium is cut off below this
 depth tempc      Temperature at center of layer, assumed to be average of layer
 boundary temperatures
  ---------------------------------------------------------------------*/

void c_twostr_set(disort_state *ds, double *bplanck, double *ch, double *chtau,
                  double *cmu, int deltam, double *dtaucpr, double *expbea,
                  double *flyr, double *gg, double *ggprim, int *layru,
                  int *lyrcut, int *ncut, int *nn, double *oprim, double *pkag,
                  double *pkagc, double radius, double *tauc, double *taucpr,
                  double *tplanck, double *utaupr) {
  ...
  /*
   * NOTE: If not using calloc() to dynamically allocate memory, then need to
   * zero-out taucpr, expbea, flyr, oprim here.
   */
   ...
   * optical depth invariant). Not worth the trouble for one-layer problems,
   * though.
   */
  *lyrcut = FALSE;
  ...
  /*
   * Calculate Chapman function if spherical geometry, set expbea and ch for
   * beam source.
   */
  if (ds->bc.fbeam > 0.) {
    CHTAU(0) = 0.;
    ...
    if (ds->flag.spher == TRUE && ds->bc.umu0 < 0.) {
      EXPBEA(0) = exp(
          -c_chapman(1, 0., tauc, ds->nlyr, ds->zd, ds->dtauc, zenang, radius));
    }
    if (ds->flag.spher == TRUE) {
      for (lc = 1; lc <= *ncut; lc++) {
        taup = TAUCPR(lc - 1) + DTAUCPR(lc) / 2.;
        CHTAU(lc) = c_chapman(lc, 0.0, taucpr, ds->nlyr, ds->zd, dtaucpr,
                              zenang, radius);
        chtau_tmp = c_chapman(lc, 0.5, taucpr, ds->nlyr, ds->zd, dtaucpr,
                              zenang, radius);
        CH(lc) = taup / chtau_tmp;
  ...
  if (!ds->flag.planck) {
    ...
    *tplanck = 0.;
    /*
     * NOTE: If not using calloc() for dynamic memory allocation, need to
     * zero-out pkag and pkagc here.
     */
  } else {
    *tplanck =
        c_planck_func2(ds->wvnmlo, ds->wvnmhi, ds->bc.ttemp) * ds->bc.temis;
...
void c_twostr_solve_bc(disort_state *ds, twostr_xyz *ts, double bplanck,
                       double *cband, double cmu, double *expbea, int lyrcut,
                       int nn, int ncut, double tplanck, double *taucpr,
                       double *kk, double *rr, int *ipvt, double *b, double *ll,
                       twostr_diag *diag) {
  int info;
  ...
  /*
   * Construct -b-, for parallel beam + bottom reflection + thermal emission at
   * top and/or bottom
   *
  ...
  for (lc = 1; lc <= nloop; lc++) {
    ...
    rpp1_m = fact1 * (YB_0D(lc + 1) + YB_1D(lc + 1) * TAUCPR(lc)) +
             fact2 * (YP_0D(lc + 1) + YP_1D(lc + 1) * TAUCPR(lc));
    rp_m = fact3 * (YB_0D(lc) + YB_1D(lc) * TAUCPR(lc)) +
           fact4 * (YP_0D(lc) + YP_1D(lc) * TAUCPR(lc));
    rpp1_p = fact1 * (YB_0U(lc + 1) + YB_1U(lc + 1) * TAUCPR(lc)) +
             fact2 * (YP_0U(lc + 1) + YP_1U(lc + 1) * TAUCPR(lc));
    rp_p = fact3 * (YB_0U(lc) + YB_1U(lc) * TAUCPR(lc)) +
           fact4 * (YP_0U(lc) + YP_1U(lc) * TAUCPR(lc));
  ...
  if (lyrcut) {
    ...
    B(++irow) = 2. * sum +
                ds->bc.albedo * ds->bc.umu0 * ds->bc.fbeam / M_PI * refflx *
                    EXPBEA(ncut) +
...
/*
  ...
  O U T P U T :  ans  : Integrated Planck function [Watts/sq m]
                         = integral (wnumlo to wnumhi) of 2h c*c
 nu*nu*nu/(exp(hc nu/(kT))-1), where h = Plancks constant, c = speed of light,
 nu = wavenumber, T=temperature,and k = Boltzmann constant
 ...
 constants, Dimensions/N.B.S., Jan. 1974
  ...
  METHOD :  For  -wnumlo-  close to  -wnumhi-, a Simpson-rule quadrature is done
            ...
            (1)  For wavenumber (wnumlo or wnumhi) small, integral(0 to wnum) is
 calculated by expanding the integrand in a power series and integrating term by
 term;
 ...
 ----------------------------------------------------------------------
        ...
        a1,2,... :  Power series coefficients
        c2       :  h*c/k, in units cm*k (h = Planck's constant, c = speed of
 light, k = Boltzmann constant) D(I)     :  Exponential series expansion of
 integral of Planck function from wnumlo (i=1) or wnumhi (i=2) to infinity ex :
 exp(-V(I)) exm      :  pow(ex,m) mmax     :  No. of terms to take in
 exponential series mv       :  multiples of 'V(i)' P(I)     :  Power series
 expansion of integral of Planck function from zero to wnumlo (i=1) or wnumhi
 (i=2) sigma    :  Stefan-Boltzmann constant (W m-2 K-4) sigdpi   :  sigma/pi
        smallv   :  Number of times the power series is used (0,1,2)
        V(I)     :  c2*(wnumlo(i=1) or wnumhi(i=2))/temperature
        vcut     :  Power-series cutoff point
        vcp      :  Exponential series cutoff points
...
c_planck_func2(double wnumlo, double wnumhi, double t) {
  register int m, n, smallv, k, i, mmax;
  static int initialized = FALSE;
  double ans, del, val, val0, oldval, exm, ex, mv, vsq, wvn, arg, hh, d[2],
      p[2], v[2];
  const double vcp[7] = {10.25, 5.7, 3.9, 2.9, 2.3, 1.9, 0.0};
...
void c_disort_state_alloc(disort_state *ds) {
   ...
   *       (such as when there is no scattering). Its first dimension needs to
   * be at least 0:ds->nstr, hence we introduce ds->nmom_nstr in the C version.
   */
  ...
  if (ds->flag.general_source == TRUE) {
    ...
    ds->gensrcu =
        c_dbl_vector(0, ds->nstr * ds->nlyr * ds->numu, "ds->gensrcu");
  } else {
    ds->gensrc = NULL;
    ds->gensrcu = NULL;
  }
  ...
  switch (ds->flag.brdf_type) {
    ...
    case BRDF_AMB:
      ds->brdf.ambrals =
          (ambrals_brdf_spec *)calloc(1, sizeof(ambrals_brdf_spec));
      if (!ds->brdf.ambrals) {
...
void c_twostr_state_alloc(disort_state *ds) {
  ...
  // 20120820ak better to just allocate this little guy regardless of the status
  // of spher. 20120820ak So I commented the following. 20120820ak  if
  // (ds->flag.spher == TRUE) {
  ds->zd = c_dbl_vector(0, ds->nlyr + 1, "ds->zd");
  // 20120820ak  }
...
int c_setout(float *sdtauc, int nlyr, int ntau, float *sutau, float *z,
             float *zout) {
  ...
  TAUINT(1) = 0.0;
  for (lc = 1; lc <= nlyr; lc++) TAUINT(lc + 1) = TAUINT(lc) + SDTAUC(lc);
...
double c_inter(int npoints, int itype, double arg, float *xarr, double *yarr,
               double *hh) {
  int iq = 0, ip = 0;
  double ynew = 0.0;
  ...
  if (arg <= XARR(1) && arg >= XARR(npoints)) {
    for (iq = 1; iq <= npoints - 1; iq++)
      if (arg <= XARR(iq) && arg >= XARR(iq + 1)) ip = iq;
    if (arg == XARR(npoints)) ip = npoints - 1;
  } else {
    if (arg > XARR(1))
      ip = 1;
    else {
  ...
  switch (itype) {
    ...
    default:
      fprintf(stderr,
              "Error, unknown itype %d (line %d, function '%s' in '%s')\n",
